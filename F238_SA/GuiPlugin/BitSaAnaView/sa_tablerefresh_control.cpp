...
void BitSA::bottomInfoShow()
{
    ...
    //Tony modify due to when mode is changed,but table is Visible is false and isHidden is no.
    if(pACPTable->isVisible()) pACPTable->refreshAcpTable();
    else if(pChannelPowerTable->isVisible()) pChannelPowerTable->refreshChannelPowerTable();
    else if(pOccupiedXdBTable->isVisible()) pOccupiedXdBTable->refreshOccupiedXdBTable();
    else if(pMultiACPTable->isVisible()) pMultiACPTable->refreshMultiAcpTable();
    else if(pSpectrumEMTable->isVisible())
    {
        if( m_user_variable->gMsComplete && !m_user_variable->gMsFirstFlag)
            pSpectrumEMTable->refreshSpectrumEMTable();
        else if(m_user_variable->gMsComplete && m_user_variable->gMsFirstFlag)
            pSpectrumEMTable->initDisplay();
    }
    else if(pSpuriousEMTable->isVisible())
    {
        ...
        if( m_user_variable->gMsComplete && !m_user_variable->gMsFirstFlag)
            ...
            pSpuriousEMTable->initDisplay();
        }
    }
    if(pMarkerTable->isVisible()) pMarkerTable->refreshMarkerTable();
#endif
}
...
void BitSA::adjustRect()
{
    ...
    */
    ...
    else
    {
        ...
        {
            pBitSA->leftWidget->setGeometry(1,65,30,648);
        ...
        {
            pBitSA->leftWidget->setGeometry(1,65,284,648);
    ...
    if(pcontrolInterface->getSweepType() == SA_SINGLE || pcontrolInterface->getTraceType() == SA_VIEW || pcontrolInterface->getMarkerFreqCountStatus())
    {
