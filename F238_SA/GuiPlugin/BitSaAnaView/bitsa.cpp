...
static qreal drawXscale,drawYscale;
static qreal drawXchange,drawYchange;
BitSA::BitSA(QWidget *parent) :
    QWidget(parent),
    singparamflag(0),
    m_ScaleMode(false),
    m_TranslationMode(false),
    m_MarkerMoveMode(false),
    mTraceAvgResetNumber(0)
{
    pBitSA = new Ui::BitSA();
    pBitSA->setupUi(this);
    ...
    qint32 iBack = SA_init_shared_memory(m_user_variable);
    if(iBack<0)
    {
...
BitSA::~BitSA()
{
    m_thread->sa_listenState = 0;
    SA_V_sem(SA_guiHandle);
    QThread::msleep(200);
    m_thread->quit();
...
void BitSA::initQssByDef()
{
#ifdef SA_F238 //define "SA_F238" F238 lock
    QString strQss;
    strQss = "";
    pBitSA->rightButton4->setStyleSheet(strQss);
#endif
#ifdef SA_F238_SMART //define "SA_F238_SMART" preset
    QString strQss;
    strQss = "#BitSA[status = \"indoor\"] #rightButton4{background-image: url(:/sa/image/btn-right-preset-normal.png);}\
              #BitSA[status = \"indoor\"] #rightButton4:pressed{background-image: url(:/sa/image/btn-right-preset-normal-click.png);}\
              #BitSA[status = \"outdoor\"] #rightButton4{background-image: url(:/sa/image/btn-right-preset-outdoor.png);}\
              #BitSA[status = \"outdoor\"] #rightButton4:pressed{background-image: url(:/sa/image/btn-right-preset-outdoor-click.png);}\
              #BitSA[status = \"night\"] #rightButton4{background-image: url(:/sa/image/btn-right-preset-night.png);}\
              #BitSA[status = \"night\"] #rightButton4:pressed{background-image: url(:/sa/image/btn-right-preset-night-click.png);}";
    pBitSA->rightButton4->setStyleSheet(strQss);
#endif
#ifdef SA_F239
    QString strQss;
...
void BitSA::saveFilePara(QString mPath,/*FILE_TYPE*/int mFileType)
{
    qint32 successFlag;
    //save File
    if(mFileType == 1)
...
void BitSA::quickSaveFilePara(QString mPath,int mFileType)
{
    qint32 successFlag;
    //save File
    if(mFileType == 1)
...
void BitSA::initPointer()
{
    ...
    connect(psa_UIRefresh_Control,SIGNAL(infoRefresh(qint32)),this,SLOT(on_uiRefresh(qint32)),Qt::DirectConnection);
...
/**
 * @brief BitSA::initThread
 * @note  initialize the sweep thread
 */
void BitSA::initThread()
{
    m_thread = new ThreadGraph(*pcontrolInterface,*m_user_variable,this);
...
//        pcontrolInterface->SetMeasureItems(m_user_variable->currentMode);
    ...
    else
    {
        ...
        qint32 iBack = pcontrolInterface->setZeroRestrainFromFile();
        if(!iBack)
        {
...
{
    int i = 0;
    if(1 == m_user_variable->greAvager[traceNo] /*|| SA_sm_config_sweep->FirstStart < 3*/)
    {
        //re average data
    ...
    if(pcontrolInterface->getCurrentMode() != SA_SPECTRUM_EMISSION_MASK_MODE /*&& m_user_variable->gMsComplete*/)
    {
        for(i = 0; i < m_user_variable->gSweepTracePoint; i++)
        {
            if(((m_user_variable->gTraceTempVal[i] - m_user_variable->gOriginalMeasureVal[i]) < 1e-1) &&
                    ((m_user_variable->gTraceTempVal[i] - m_user_variable->gOriginalMeasureVal[i]) > -1e-1) /*&& m_user_variable->gAverageNo[traceNo] != 0*/)
            {
                continue;
            }
            ////Johnson 190417
        ...
        memcpy(m_user_variable->gTraceTempVal,m_user_variable->gOriginalMeasureVal,sizeof(qreal) * m_user_variable->gSweepTracePoint );
    }
    ...
    else
    {
        ...
        {
//            mTraceAvgResetNumber ++;
            if(mTraceAvgResetNumber > 1)
            {
                mTraceAvgResetNumber = 1;
                m_user_variable->gAverageNo[traceNo]++;
            }
        }
...
void BitSA::graphRefreshFunc()
{
    ...
    if(this->isHidden() && !bTableWidgetInitComplete)
    {
        if(m_user_variable->currentMode == SA_SPECTRUM_EMISSION_MASK_MODE||\
                m_user_variable->currentMode == SA_SPURIOUS_EMISSION_MASK_MODE)
        {
            //DelayFunc(10);
        ...
        {
            SA_sm_config_sweep->SweepType = m_user_variable->gSweepType;
        }
    ...
    //Johnson 190621
    if(m_user_variable->gMarkerFreqCount)
    {
    ...
    if(m_ScaleMode)
    {
        psa_UIRefresh_Control->refreshScalePixmap(drawXscale,drawYscale);
    }else if(m_TranslationMode)
    {
        psa_UIRefresh_Control->refreshTranslationPixmap(drawXchange,drawYchange);
    }else if(!m_ScaleMode && !m_TranslationMode)//normal
    {
        psa_UIRefresh_Control->refreshPixmap();
    }
#else
    //Johnson 20200826
    pcontrolInterface->refreshDataAfterCompensation();
    psa_UIRefresh_Control->refreshPixmap();
#endif
...
void BitSA::refreshSpecEMData()
{
    ...
    if(index == 0)
    {
        freq = halfspanFreq  - (m_user_variable->gSpecEMData[index].start + m_user_variable->gSpecEMRefChBw/2);
    ...
    {
        freq = halfspanFreq  - (m_user_variable->gSpecEMData[index-1].stop+ m_user_variable->gSpecEMRefChBw/2);
        lowerStart = freq / setpFreq;
        if(lowerStart*setpFreq != freq)
        {
            lowerStart = lowerStart + 1;
        }
        freq = halfspanFreq  - (m_user_variable->gSpecEMData[index-1].start + m_user_variable->gSpecEMRefChBw/2);
        lowerStop = freq / setpFreq;
        if(lowerStop * setpFreq == freq)
        {
            lowerStop = lowerStop - 1;
...
void BitSA::processRawData()
{
    ...
    else if(singparamflag == 3)//Refresh draw curve
    {
        for (i = 0; i < NUM_TRACE; i++ )
        {
            ...
            case SA_MAX_HOLD:
            case SA_MIN_HOLD:
                if((m_user_variable->gTraceTypeChangeStatus[i] == 1))
                {
                    SA_sm_config_sweep->uMinMaxHoldComplete = 0;
                    m_user_variable->uMinMaxHoldReadComplete = 0;
                    memcpy(m_user_variable->gTraceMeasureVal[i], m_user_variable->gOriginalMeasureVal, sizeof(qreal) * m_user_variable->gSweepTracePoint );
//                    m_user_variable->gTraceTypeChangeStatus[i] = 0;
                }
                break;
            }
        }
    ...
    else
    {
        for (i = 0; i < NUM_TRACE; i++ )
        {
            if(m_user_variable->gTraceView[i] == SA_OFF)
                continue;
            if(SA_sm_config_sweep->FirstStart == 0)
            {
                m_user_variable->gTraceTypeChangeStatus[i] == 1;
            }
            switch (m_user_variable->gTraceType[i])
            {
            case SA_CLEAR_WRITE:
            case SA_TRACE_AVE:
                ...
                break;
            case SA_MAX_HOLD:
                if((!SA_sm_config_sweep->uMinMaxHoldComplete && (m_user_variable->gTraceTypeChangeStatus[i] == 1)) || SA_sm_config_sweep->FirstStart < 5)
                {
                ...
                {
                    if((!m_user_variable->uMinMaxHoldReadComplete && (m_user_variable->gTraceTypeChangeStatus[i] == 1)) || SA_sm_config_sweep->FirstStart < 5)
                    {
                        memcpy( m_user_variable->gTraceMeasureVal[i], m_user_variable->gOriginalMeasureVal, sizeof(qreal) * m_user_variable->gSweepTracePoint );
...
void BitSA::mousePressEvent(QMouseEvent *event)
{
    ...
    SA_Dialog_DisplaySetup *mSweepSetup;
    ...
    if(event->x() >= CenterDrawRect.x() && event->y() >= CenterDrawRect.y())
    {
        ...
        else if(mSetupDialog && m_user_variable->PrimaryAction == SA_SWEEP)
        {
            mSweepSetup = (SA_Dialog_DisplaySetup *)mSetupDialog;
            if(mSweepSetup->mDoorControl)
            {
                mSweepSetup->gateSweepDisplay();
            }
        }
#endif
    }
...
void BitSA::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if(1 == SA_sm_config_sweep->ListenState || pcontrolInterface->getMarkerFreqCountStatus())
    {
        psa_UIRefresh_Control->refreshPixmap();
        repaint(CenterDrawRect);
    }
...
void BitSA::keyPressEvent(QKeyEvent *event)
{
    ...
    if(event->key() == Qt::Key_PowerOff/* || Qt::Key_Q*/)
    {
...
//                m_user_variable->gLockStatus == SA_OFF;
    ...
    if(event->key() == Qt::Key_F4 || event->key() == Qt::Key_F3)
    {
        if(SA_SINGLE == m_user_variable->gSweepType || pcontrolInterface->getMarkerFreqCountStatus())
        {
                psa_UIRefresh_Control->refreshPixmap();
                repaint(CenterDrawRect);
        }
...
void BitSA::initWidget(void)
{
    ...
    connect(mFileManager,SIGNAL(saveFileSig(QString,int)),this,SLOT(saveFilePara(QString,int)));
...
#ifdef SA_F238_SMART //define "SA_F238" F238 lock
    pBitSA->verticalLayout->removeWidget(pBitSA->rightButton4);
    pBitSA->verticalLayout->removeWidget(pBitSA->rightButton9);
    pBitSA->verticalLayout->removeItem(pBitSA->verticalSpacer);
    pBitSA->verticalLayout->removeWidget(pBitSA->rightButton10);

    pBitSA->verticalLayout->addWidget(pBitSA->rightButton4);
    pBitSA->verticalLayout->addWidget(pBitSA->rightButton9);
    pBitSA->verticalLayout->addItem(pBitSA->verticalSpacer);
    pBitSA->verticalLayout->addWidget(pBitSA->rightButton10);
#endif
    return ;
}
...
void BitSA::loadstylesheet(int stylemode)
{
    ...
    {
        setNoticePara(NOTIC_MESSAGE,EXECUTE_ERR_EXCEPTION,SA_QssFile_Read);
        errorOutput();
    }
    return;
}
...
void BitSA::on_rightButton1_clicked()
{
    ...
    {
        SA_Dialog_MarkerSingle* mMarkerTabSetup = NULL;
        mMarkerTabSetup = (SA_Dialog_MarkerSingle*)mSetupDialog;
        mMarkerTabSetup->refreshState();
        return;
    }
...
void BitSA::on_rightButton9_clicked()//Home
{
    //
    m_user_variable->bShiftKeyPressed = false;
    MenuClose();
    SA_V_sem(SA_homeuiHandle);
    return ;
}

void BitSA::on_rightButton10_clicked() //Zoom
{
    menuClose();
    setZoomIn();
...
//            gettimeofday(&tpstart,NULL);
            if(SA_MARKER != m_user_variable->PrimaryAction && SA_MARKER_ALLSET != m_user_variable->PrimaryAction)
            {
                mSetupDialog->setVisible(true);
                mInteDialog->initDisplay();
            }
...
void BitSA::showMkrMenu(qint32 index)
{
    ...
    pcontrolInterface->setMarkerNoiseStatus(pcontrolInterface->getMarkerNoiseStatus());
    if(FullStatus)
    {
                ...
                if(pcontrolInterface->getMarkerType(i) == REF)
                {
                    if(!m_user_variable->gSpanMode)
                    {
                        ...
                        if(m_user_variable->gMarkerFreqCount)
                        {
                            ...
                            qreal value = 0.;
                            qreal markoffSignl = m_user_variable->gMarkerRefFreq[m_user_variable->gMarkerSelect] - m_user_variable->gSignalFreq;
                            qreal MarkoffCenter = m_user_variable->gMarkerRefFreq[m_user_variable->gMarkerSelect] - m_user_variable->gCenterFreq;
                            if(fabs(MarkoffCenter) <= m_user_variable->gRbw/2)
                            {
                                //Tony19-12-19 if value = gFreqCount ,then value = 0 when marker on center but isn't 0;
                                value = m_user_variable->gCenterFreq;
                            }
                            else value = (qreal)m_user_variable->gFreqCount + markoffSignl;
                            ...
                            strFreq = QString::number(value,'d',3);
                            if(minusFlag) strFreq = "-" + strFreq + " Hz";
                            else strFreq = strFreq + " Hz";
                        }else
                        */
                        {
...
//        pcontrolInterface->setMarkerNoiseStatus(SA_OFF,i);
//        pcontrolInterface->setMarkerFreqCountStatus(SA_OFF,i);
    }
    pcontrolInterface->setPeakTrackStatus(SA_OFF);
    ...
    if(pBitSA->topButton7->isChecked() || FullStatus)
    {
        if(mSetupDialog && m_user_variable->PrimaryAction == SA_MARKER_TAB)
        {
            SA_Dialog_MarkerSingle* mMarkerTabSetup = NULL;
            mMarkerTabSetup = (SA_Dialog_MarkerSingle*)mSetupDialog;
            mMarkerTabSetup->initDisplay();
        }else if(mSetupDialog && mSetupDialog && m_user_variable->PrimaryAction == SA_MARKER)
        {
            SA_Dialog_MarkerSetup* mMarkerSetup = NULL;
            mMarkerSetup = (SA_Dialog_MarkerSetup*)mSetupDialog;
...
void BitSA::on_infoRefresh(qint32 mType, qint32 mIndex)
{
    ...
    if(pcontrolInterface->getSweepType() == SA_SINGLE || pcontrolInterface->getMarkerFreqCountStatus())
    {
...
{
    qint32 colorMode = m_user_variable->gDisplayMode;
    ...
    for(qint32 i=0;i<TRACE_CNT_TOTAL;i++)
    {
        ...
        if(i == trace && SA_ON == status)
        {
            ...
            strbg = QString("#lblbgT%1{background-color: %2;}\n").arg(QString::number(i+1)).arg(makeColorString(mTraceColor[i]));
            strTrace = QString("#lblT%1{color:%2;font:75 13px \"Arial\";font-weight:bold;}\n").arg(QString::number(i+1)).arg("rgb(0,0,0)");
        }
        else
        {
            strbg = QString("#lblbgT%1{background-color:rgba(0,0,0,0);}\n").arg(QString::number(i+1));
            strTrace = QString("#lblT%1{color:%2;font:75 13px \"Arial\";font-weight:bold;}\n").arg(QString::number(i+1)).arg(makeColorString(mTraceColor[i]));
        }
...
QString BitSA::makeColorString(const QColor &color, const QString type)
{
    if(type == "RGBA") {
...
void BitSA::on_uiRefresh(qint32 index)
{
    if(index == SA_INFO_GATEDLY)
        pcontrolInterface->setEdgeTrigDely(m_user_variable->gEdgTrigDely);
    else if(index == SA_INFO_GATELEN)
        pcontrolInterface->setEdgeTrigDoorWidth(m_user_variable->gEdgTrigDoorWdth);
    else
    {
        if(SA_INFO_MARKER_INDEX == index)
            pcontrolInterface->setMarkerNoiseStatus(pcontrolInterface->getMarkerNoiseStatus());
...
qint32 BitSA::setParaForSEM()
{
    ...
    //Tony 20-02-17
    qint32 freqTotalCnt;
    if(m_user_variable->gSpecEMMaskType == SA_SPECEM_DEFAULT)
    {
        freqTotalCnt = SA_SPECEM_FREQ_CNT+1;
    }else
    {
        freqTotalCnt = SA_SPECEM_FREQ_CNT_5GNR+1;
    }

    if(!m_user_variable->gMsFirstFlag)
    {
        freqIndex = (freqIndex +1)%(freqTotalCnt);
    ...
    else
    {
        ...
        if(m_user_variable->gSpecEMMaskType == SA_SPECEM_DEFAULT)
        {
            defaultSpan = 25000000LL + pcontrolInterface->getSpecEMRefChBw();
        }else
        {
            defaultSpan = 280000000LL + pcontrolInterface->getSpecEMRefChBw();
        }
        //Tony 19005
        bParaLoad = true;
        pcontrolInterface->blockSignals(true);
        ...
        pcontrolInterface->setCenterFreq(m_user_variable->gCenterFreq);
        pcontrolInterface->setSpan(defaultSpan);
    ...
    {
    case 0:
        mRBW = pcontrolInterface->getSpecEMRefRbw();
//        if(m_user_variable->gSpecEMMaskType == SA_SPECEM_DEFAULT)
//        {
...
//    }
    ...
    pcontrolInterface->setStartFreq(mStartFreq);
    pcontrolInterface->setStopFreq(mStopFreq);

//    pcontrolInterface->setCenterFreq(mCenter);
//    pcontrolInterface->setSpan(mSpan);
    pcontrolInterface->setAutoSweepTimeStatus(SA_AUTO);
//    pcontrolInterface->setSweepTime(mSweepTime);
    pcontrolInterface->setAttenValue(m_user_variable->gAttenValue);
    bParaLoad = false;
    m_user_variable->gAutoRepeat = 1;
    m_user_variable->gIgnoreLimit = 0;
    pcontrolInterface->blockSignals(false);
...
#endif
    SA_sm_config_sweep->VBW = uVbwValue;
#if USABLE
...
*/
 ...
 bool BitSA::event(QEvent *event)
 {
     ...
     {
         if(pcontrolInterface->getSweepType() == SA_SINGLE) return true;
         QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
         QList<QTouchEvent::TouchPoint> touchPoints = touchEvent->touchPoints();
         if(!psa_UIRefresh_Control->IsPixmapRecPoint(touchPoints.first().pos().x()- CenterDrawRect.x(),touchPoints.first().pos().y()- CenterDrawRect.y()))
         {
         ...
         if (touchPoints.count() == 2)
         {
             if(!m_TranslationMode)
             {
                 ...
                 {
                     m_ScaleMode = true;
                     psa_UIRefresh_Control->firstTouch = 1;
                 }
             }else
             {
                 break;
             ...
             qreal OriXlen,PresXlen,OriYlen,PresYlen;
             PresXlen = qAbs(QLineF(touchPoint0.pos(), touchPoint1.pos()).dx());
             PresYlen = qAbs(QLineF(touchPoint0.pos(), touchPoint1.pos()).dy());
             OriXlen = qAbs(QLineF(touchPoint0.startPos(), touchPoint1.startPos()).dx());
             OriYlen = qAbs(QLineF(touchPoint0.startPos(), touchPoint1.startPos()).dy());
             OriXlen = OriXlen > 20 ? OriXlen : 20;
             OriYlen = OriYlen > 20 ? OriYlen : 20;
             PresXlen = PresXlen > 20 ? PresXlen : 20;
             PresYlen = PresYlen > 20 ? PresYlen : 20;
             drawXscale = PresXlen / OriXlen ;
             drawYscale = PresYlen / OriYlen ;
             if (touchEvent->touchPointStates() & Qt::TouchPointReleased)
             {
                 if(m_user_variable->gSpanFreq)
                 {
                     qreal span = pcontrolInterface->getSpan()/drawXscale;
                     pcontrolInterface->setSpan(span);
                 }else
                 {
                     qreal time = pcontrolInterface->getSweepTime()/drawXscale;
                     pcontrolInterface->setSweepTime(time);
                 }

                 qreal midDivScale = pcontrolInterface->getRefLevel()+pcontrolInterface->getRefOffset() - pcontrolInterface->getScaleDiv()*5;
                 qreal divScale = pcontrolInterface->getScaleDiv()/drawYscale;
                 pcontrolInterface->setRefLevel(midDivScale+5*divScale-pcontrolInterface->getRefOffset());
                 pcontrolInterface->setScaleDiv(divScale);
             }
         }
         else if(touchPoints.count() == 1)
         {
             if(!m_MarkerMoveMode && !m_ScaleMode)
             {
                 const QTouchEvent::TouchPoint &touchPointOne = touchPoints.first();
                 if(m_TranslationMode)
                 {
                     drawXchange = touchPointOne.pos().x() - touchPointOne.startPos().x();
                     drawYchange = touchPointOne.pos().y() - touchPointOne.startPos().y();
                 }else
                 {
                     qint32 flag = psa_UIRefresh_Control->IsMarkerPoint((qint32)touchPointOne.pos().x()- CenterDrawRect.x(), (qint32)touchPointOne.pos().y()- CenterDrawRect.y());
                     if(!flag)
                     {
                         drawXchange = touchPointOne.pos().x() - touchPointOne.startPos().x();
                         drawYchange = touchPointOne.pos().y() - touchPointOne.startPos().y();
                         if(qAbs(drawXchange)>=1 && qAbs(drawYchange) >= 1)
                         {
                              psa_UIRefresh_Control->firstTouch = 1;
         ...
         if(QEvent::TouchEnd == event->type())
         {
             if(m_TranslationMode)
             {
                qreal AmptChange = drawYchange/(CenterDrawRect.height()-SA_TOPDIS-SA_BOTTOMDIS)*pcontrolInterface->getScaleDiv()*10;
                pcontrolInterface->setRefLevel(pcontrolInterface->getRefLevel()+AmptChange);
                if(m_user_variable->gSpanFreq)
                {
                    qreal FreqChange = drawXchange/(CenterDrawRect.width()-SA_LEFTDIS-SA_RIGHTDIS)*pcontrolInterface->getSpan();
                    pcontrolInterface->setCenterFreq(pcontrolInterface->getCenterFreq()-FreqChange);
                }//zero span doesn't set anything
             }
             m_TranslationMode = false;
             m_ScaleMode = false;
             m_MarkerMoveMode = false;
