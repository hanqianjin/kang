...
#if SPUREMODIFY
    ui->LedPeakDefinition->setVisible(false);
    ui->LedPeakThreshold->setVisible(false);
    ui->LedAtten->setVisible(false);
    ui->label_10->setVisible(false);
    ui->label_13->setVisible(false);
    ui->label_15->setVisible(false);

    ui->label_3->setVisible(false);
    ui->pb_Report_Mode1->setVisible(false);
    ui->pb_Report_Mode2->setVisible(false);
    ui->label_2->setVisible(false);
    ui->LedSpurIndex->setVisible(false);
    ui->line_2->move(18,170);
    ui->pb_RangePara_1->move(40,190);
    ui->pb_RangePara_2->move(140,190);
    ui->label_16->move(18,240);
    ui->LedRangeIndex->move(94,240);
    ui->label_17->move(18,290);
    ui->pb_RangeOn->move(94,290);
    ui->stackedWidget->move(0,340);

    ui->label_14->setVisible(false);
    ui->LedSweepTime->setVisible(false);
#endif
}
...
{
    QString qss = QString("QLineEdit{outline: none;border:2px solid rgba(162,115,0,1);padding:6px 6px 5px 8px;}");
    if(mCurEdit<0 && mBoxList.contains(mCurEdit))
    {
        mBoxList.value(mCurEdit)->setStyleSheet("");
    }else if(mCurEdit>0&& mEditList.contains(mCurEdit))
...
void SA_Dialog_SpurEM::on_VBWBox_currentIndexChanged(qint32 index)
{
    qreal vbw;//,rbw,ratio;
    qint32 mSize = sizeof(SA_u32BWTable)/4;
    if(index >= (mSize - 1))
