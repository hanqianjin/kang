...
void sa_widget_SpuriousEMTable::initDisplay()
{
    ...
    pen.setWidth(2);
    painter.setPen(pen);
    qreal widthStep = (qreal)  (SpuriousEMTablePixmap.width() - 430) / (ColumNum -4);
    RowPos[0]= 0;
...
//    RowWidth[4] = 35;
    ...
    //Title
    RowContent[0] = "Range";
    RowContent[1] = "Frequency Range";
    RowContent[2] = "RBW";
    RowContent[3] = "Peak Frequency";
    RowContent[4] = "Peak Amplitude";
    RowContent[5] = "Limit";
    RowContent[6] = "Attenuation";
    RowContent[7] = "Pass/Fail";
    for(qint32 i=0;i<ColumNum;i++)
    {
...
void sa_widget_SpuriousEMTable::refreshSingleRangeTable()
{
    qint32 colorMode = globalUserVariable->gDisplayMode;
    QColor fontColor = TableFontColor[colorMode];
    QColor curFontColor = TableSelectFontColor[colorMode];

    qint32 mValuePassFlag = 1;
    qint32 ColumNum = 8,mCurRangeIndex;
    qint32 mFocusSpur;
    QPainter painter(&SpuriousEMTablePixmap);
    QString RowContent[ColumNum];
    qint32  RowWidth[ColumNum],RowPos[ColumNum];
    ...
    globalUserVariable->gSpurEMTotalSpurCnt = 5;

    mFocusSpur = mControl->getSpurEMCurRangeIndex();
    //draw content
    for(qint32 row =0;row < globalUserVariable->gSpurEMTotalSpurCnt/*mCurRangeIndex+1*/;row++)
    {
        if(mFocusSpur == row)
        ...
        rowIndex++;
        RowContent[0] = QString::number(row + 1); //Range
        RowContent[1] = QString("%1-%2").arg(globalUserVariable->freqValueToQstring(globalUserVariable->gSpurEMPara[row].startFreq,2)).arg(globalUserVariable->freqValueToQstring(globalUserVariable->gSpurEMPara[row].stopFreq,2)); //Freq Range
        RowContent[2] = globalUserVariable->freqValueToQstring(globalUserVariable->gSpurEMPara[row].RBW,2); //Rbw

        RowContent[5] = QString::number(globalUserVariable->gSpurEMSpur[row].limit,'f',2)+" dBm";//Limit
        RowContent[6] = QString().sprintf("%.2f dB",globalUserVariable->gAttenValue);//Attenuation
        if(SA_SPUREM_AVE_REPEAT == mControl->getSpurEMAveType())
        {
            if(row==mCurRangeIndex)
            {
                if(mControl->getSpurEMDisplay(row))
                {
                    RowContent[3] = globalUserVariable->freqValueToQstring(globalUserVariable->gSpurEMSpur[row].freq);//Peak Freq
                    RowContent[4] = QString::number(globalUserVariable->gSpurEMSpur[row].value)+" dBm"; //Peak Ampt
                    if(globalUserVariable->gSpurEMSpur[row].testValue==SA_FAIL)
                    {
                        RowContent[7] = "Fail";
                    }else {
                        RowContent[7] = "Pass";
                ...
                {
                    RowContent[3] = "-- --";//Peak Freq
                    RowContent[4] = "-- --"; //Peak Ampt
                    RowContent[7] = "-- --";
                }
            }else
            {
...
void sa_widget_SpuriousEMTable::refreshSingleRangeTable()
{
    ...
    if(mCurSpurIndex >= (SA_SPUREM_PAGESPUR_CNT -1))
    {
        ...
        mFocusSpur = rowCnt -1;
        baseIndex = mCurSpurIndex + 1 - rowCnt;
    }else{
    ...
    for(qint32 row =0;row < rowCnt;row++)
    {
        if(mFocusSpur == row)
            ...
            painter.setPen(pen);
        rowIndex++;
        RowContent[0] = QString::number(baseIndex+ row + 1);
        RowContent[1] = QString::number(globalUserVariable->gSpurEMSpur[spurIdList.at(baseIndex+row)].rangeIndex);
        RowContent[2] = globalUserVariable->freqValueToQstring(globalUserVariable->gSpurEMSpur[spurIdList.at(baseIndex+row)].freq);
        RowContent[3] = QString::number(globalUserVariable->gSpurEMSpur[spurIdList.at(baseIndex+row)].value)+" dBm";

        if(globalUserVariable->gSpurEMSpur[spurIdList.at(baseIndex+row)].testValue==SA_FAIL)
        {
            RowContent[4] = "F";
        }else {
            RowContent[4] = "";
        ...
        RowContent[5] = QString::number(globalUserLimit->gSpurLimit[globalUserVariable->gSpurEMSpur[spurIdList.at(baseIndex+row)].xPtIndex].y())+" dBm"; //limit error  limit change
        for(qint32 i=0;i<ColumNum;i++)
        {
            if(i == 4)
            {
...
void sa_widget_SpuriousEMTable::spurSearch()
{
    ...
    qreal peakVal = globalUserVariable->gTraceMeasureVal[globalUserVariable->gTraceSelect][peakPos];
...
//    }
    ...
    globalUserVariable->gSpurEMSpur[curRangeIndex].freq = globalUserVariable->gStartFreq + (globalUserVariable->gStepFreq * peakPos);
    globalUserVariable->gSpurEMSpur[curRangeIndex].value = peakVal;
    globalUserVariable->gSpurEMSpur[curRangeIndex].xPtIndex = peakPos;
    globalUserVariable->gSpurEMSpur[curRangeIndex].rangeIndex = curRangeIndex + 1;
    globalUserVariable->gSpurEMSpur[curRangeIndex].limit = globalUserLimit->gSpurLimit[peakPos].y();
    return;
}
#else
void sa_widget_SpuriousEMTable::spurSearch()
...
void sa_widget_SpuriousEMTable::judgeSpurEMTestResult()
{
    ...
    globalUserVariable->gSpurEMSpur[curRange].testValue = SA_PASS;

    if(globalUserVariable->gSpecEMViewStatus[curRange] == SA_OFF) return;

    mLimit = globalUserLimit->gSpurLimit[globalUserVariable->gSpurEMSpur[curRange].xPtIndex].y();
    mAmpt = globalUserVariable->gSpurEMSpur[curRange].value;
    if(mAmpt > mLimit)
    {
        globalUserVariable->gSpurEMSpur[curRange].testValue = SA_FAIL;
    }
