...
typedef struct
{
    ...
    qint64 gChannelBandwidth;
    qint64 gChannelSpacing;
    qreal gOccupiedBW;
    qreal XdB;
...
#ifdef SAMPLE_LIMIT
    qreal gUpperLimitAmpt;
    qreal gLowerLimitAmpt;
    qint32 gUpperLimitStatus;
    qint32 gLowerLimitStatus;
    qreal gAtlLimitdBc;
...
typedef struct
{
    _CONFIG_FREQ_SPAN_   gConfig_FreqSpan;/*频率扫宽配置*/
    _CONFIG_BW_          gConfig_BW;/*带宽配置*/
    _CONFIG_AMPLITUDE_   gConfig_Amplitude;/*幅度配置*/
    _CONFIG_TRACE_       gConfig_Trace;/*迹线配置*/

    quint64              gCenterFreq = 0;/*中心频率*/
    quint32              mSaveLoad_CurrentMode = 0;/*当前状态--当前哪种MeasureMode*/
    _CONFIG_SHR_         gConfig_ShareMem;/*共享内存配置--unused*/

    _CONFIG_STATE_MSDATA_ measureModeParam;/*测量模式对应的配置参数*/
}_SA_ALL_CONF_,*p_SA_ALL_CONF_;
/*
@ author:   johnson
@ date:     2020-11-19
*/
typedef struct
{
    _CONFIG_FREQ_SPAN_  gFreqSpan;
    _CONFIG_SWEEP_      gSweep;
    _CONFIG_BW_         gbw;
}_MEASURE_STATE_;
