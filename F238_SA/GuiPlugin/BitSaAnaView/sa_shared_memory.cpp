...
SA_PALL_NOTIC_CODE SA_sm_notic_code = NULL;
...
qint32 SA_init_shared_memory(user_variable *globalUserVariable)
{
    ...
    SA_sm_info.sm_noticecode_id = 0;
    SA_sm_info.sm_noticecode_addr = NULL;
  ...
  //190524 Johnson
    ...
    {
        setNoticePara(WARNING_EXCEPTION_MESSAGE,EXECUTE_ERR_EXCEPTION,SA_ShmGetID_bsavisi);
        errorOutput(REDIRECT_YES,WRITESHM_NO);
        return -1;
    }
    else
    {
        ...
        {
            setNoticePara(WARNING_EXCEPTION_MESSAGE,EXECUTE_ERR_EXCEPTION,SA_ShmGetAddr_bsavisi);
            errorOutput(REDIRECT_YES,WRITESHM_NO);
            return -1;
        }
  ...
  // config temper calc
    ...
    //Johnson 091015 create noticcode shm
    SA_sm_info.sm_noticecode_id = shmget((key_t)SM_KEY_NOTICE,sizeof(SA_ALL_NOTIC_CODE),IPC_CREAT|0666);
    if  (-1 == SA_sm_info.sm_noticecode_id)
    {
    ...
    {
        SA_sm_info.sm_noticecode_addr = shmat(SA_sm_info.sm_noticecode_id, (void*)0, 0);
        if (SA_sm_info.sm_noticecode_addr == (void*)-1)
        {
            setNoticePara(WARNING_EXCEPTION_MESSAGE,EXECUTE_ERR_EXCEPTION,SA_ShmGetAddr_noticecode);
            errorOutput(REDIRECT_YES,WRITESHM_NO);
            return -1;
        }
        SA_sm_notic_code = (SA_ALL_NOTIC_CODE *) SA_sm_info.sm_noticecode_addr;
    }
