...
#include "ui_sa_dialog_quicksave.h"
#include "define.h"
#include "sa_dialog_warning.h"
#include "sa_dialog_error.h"
#include "sa_applicationcontrol.h"
SA_Dialog_QuickSave::SA_Dialog_QuickSave(controlInterface *interface, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SA_Dialog_QuickSave)
{
    ui->setupUi(this);
    mParent = parent;
    mControl = interface;
    ...
    mKeyboard = new SA_Dialog_VirtualKeyboard(this);
    mKeyboard->setGeometry(QRect(30, 100, 950, 290));
    mKeyboard->setProperty("status",qssMode[mControl->GetDisplayMode()]);
...
void SA_Dialog_QuickSave::setDisplayMode(QString mode)
{
    setProperty("status",mode);
    mKeyboard->setProperty("status",mode);
}
SA_Dialog_QuickSave::~SA_Dialog_QuickSave()
{
//    delete mScreenShots;
...
void SA_Dialog_QuickSave::keyPressEvent(QKeyEvent *event)
{
    ui->inputEdit->setFocus();
...
void SA_Dialog_QuickSave::initVariable()
{
    savePath = SA_DEFAULT_SCREENSHOTS_PATH;
...
void SA_Dialog_QuickSave::initDisplay()
{
    ui->inputEdit->setText(strInput);
}

void SA_Dialog_QuickSave::initWidget()
{
    qint32 x,y;
    x = (1280 - this->width())/2;
    y = (800 - this->height())/2;
...
void SA_Dialog_QuickSave::initFunc()
{
    mScreenShots = ((QWidget*)mParent->parent())->grab(QRect(SCREEN_CAPTURE_STARTX, SCREEN_CAPTURE_STARTY, SCREEN_CAPTURE_WIDTH, SCREEN_CAPTURE_HEIGHT));
    return;
}

QString SA_Dialog_QuickSave::getDefaultFileName(QString mSavePath)
{
    QDir mDir(mSavePath);
    QStringList filter;
...
bool SA_Dialog_QuickSave::SaveScreenShots()
{
    ...
    if(checkID == 1)
    {
        SA_Dialog_Error message("Warning","Folder name contains illegal characters!\nPlease create folder with another name.",this);
        message.setWindowFlags(Qt::WindowStaysOnTopHint|Qt::Window | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
        message.exec();
        return false;
    }else if(checkID == 2)
    {
        SA_Dialog_Warning message("Notice","The beginning will cause fill hiding.Continue?",this);
        message.setWindowFlags(Qt::WindowStaysOnTopHint|Qt::Window | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
...
//        }
    ...
    if(file.isFile())
    {
        SA_Dialog_Warning message("Notice","The file already exists.Is it covered?",this);
        message.setWindowFlags(Qt::WindowStaysOnTopHint|Qt::Window | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
        if(message.exec() != QDialog::Accepted)
        {
            return false;
    ...
    {
        //save faild
        SA_Dialog_Error message("Warning","Save failed!\nPlease check memory.",this);
        message.setWindowFlags(Qt::WindowStaysOnTopHint|Qt::Window | Qt::CustomizeWindowHint | Qt::FramelessWindowHint);
        message.exec();
        return false;
    }
 ...
 */
qint32 SA_Dialog_QuickSave::checkFileName(QString mFile)
{
    qint32 cnt = mFile.size();
    qint32 pos = mFile.lastIndexOf('.');
    QString strSuffix;
    //Folder name contains illegal characters!
    qint32 size = sizeof(Sa_GucRegularString);
    for(qint32 i = 0;i < size;i++)
    {
        if(mFile.contains(Sa_GucRegularString[i]))
            return 1;
    }
...
void SA_Dialog_QuickSave::on_pb_cancel_clicked()
{
    this->close();
    return;
}

void SA_Dialog_QuickSave::on_pb_ok_clicked()
{
    if(SaveScreenShots())
    {
        this->close();
...
void SA_Dialog_QuickSave::backSpace()
{
...
void SA_Dialog_QuickSave::clearAll()
{
    ui->inputEdit->clear();
    strInput = ui->inputEdit->text();
    return;
...
void SA_Dialog_QuickSave::on_pb_clear_clicked()
{
    clearAll();
    return;
}
