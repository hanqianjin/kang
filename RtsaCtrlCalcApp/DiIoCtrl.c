...
#define PARAM_PHY_ADDR      0X84000000
#define CONFIG_PHY_ADDR     0X86000000
//==============================================================================
// Types
...
#define PARAM_SIZE      PAGE_SIZE
#define CONFIG_SIZE     ((PAGE_SIZE << 1) + PAGE_SIZE)
#define MMAP_SIZE		(CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + CONFIG_SIZE)
...
DiInt32 MemBitRead8(DiSession devHandle, DiUInt32 addr, DiPUInt8 value)
{
	...
	else if(PARAM_PHY_ADDR <= addr && addr <= PARAM_PHY_ADDR + PARAM_SIZE)
        {
            offset = addr - PARAM_PHY_ADDR ;
            *value = *(volatile DiUInt8 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset);
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*value = *(volatile DiUInt8 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset);
		}
    else
        {
            return -1;
...
DiInt32 MemBitRead16(DiSession devHandle,DiUInt32 addr,DiPUInt16 value)
{
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *value = *(volatile DiUInt16 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset);
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*value = *(volatile DiUInt16 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset);
		}
    else
        {
            return -1;
...
{  
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *value = *(volatile DiUInt32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset);
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*value = *(volatile DiUInt32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset);
		}
    else
        {
            return -1;
...
Others: 		NULL
*************************************************/
DiInt32 MemBitReadFloat32(DiSession devHandle,DiUInt32 addr,DiPReal32 value)
{  
#if BIT_DEBUG
    return 0;
#endif
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        {
            offset = addr - CPLD_PHY_ADDR ;
            *value = *(volatile DiReal32*)(pl_map_base + offset);
        }
    else if(DSP_PHY_ADDR <= addr && addr <= DSP_PHY_ADDR + DSP_SIZE)
        {
            offset = addr - DSP_PHY_ADDR ;
            *value = *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + offset);
        }
    else if(DETECTOR_PHY_ADDR <= addr && addr <= DETECTOR_PHY_ADDR + DETECTOR_SIZE)
        {
            offset = addr - DETECTOR_PHY_ADDR ;
            *value = *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + offset);
        }
	else if(PARAM_PHY_ADDR <= addr && addr <= PARAM_PHY_ADDR + PARAM_SIZE)
        {
            offset = addr - PARAM_PHY_ADDR ;
            *value = *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset);
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*value = *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset);
		}
    else
        {
            return -1;
...
{  
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *value = *(volatile DiUInt64 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset);
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*value = *(volatile DiUInt64 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset);
		}
    else
        {
            return -1;
...
DiInt32 BitMoveOutFloat32(DiSession devHandle, DiUInt32 addr, DiBusSize length, DiPReal32 buf32)
{
    ...
    for(i = 0;i < length; i++)
        {
            iBack = MemBitReadFloat32(devHandle, tempaddr, buf32 + i);
            if(iBack < 0)
                {
                    printf("block read failed at %d times\n",i);
                    return iBack;
...
DiInt32 BitWrite8(DiSession devHandle, DiUInt32 addr, DiUInt8 value)
{
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *(volatile DiUInt8 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset) = value;
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*(volatile DiUInt8 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset) = value;
		}
    else
        {
            return -1;
...
DiInt32 BitWrite16(DiSession devHandle,DiUInt32 addr,DiUInt16 value)
{
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *(volatile DiUInt16 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset) = value;
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*(volatile DiUInt16 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset) = value;
		}
    else
        {
            return -1;
...
DiInt32 BitWrite32(DiSession devHandle,DiUInt32 addr,DiUInt32 value)
{
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        ...
        {
            offset = addr - PARAM_PHY_ADDR ;
            *(volatile DiUInt32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset) = value;
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*(volatile DiUInt32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset) = value;
		}
    else
        {
            return -1;
...
DiInt32 BitReadFloat32(DiSession devHandle,DiUInt32 addr,DiPReal32 value)
{
    ...
    iBack = MemBitReadFloat32(devHandle, addr, value) ;
    //iBack = MemBitRead32(devHandle, addr, value) ;//due to the PL, must be read twice
    ...
    if(CPLD_PHY_ADDR <= addr && addr <= CPLD_PHY_ADDR + CPLD_SIZE)
        {
            offset = addr - CPLD_PHY_ADDR ;
            //printf("offset:%d\n", offset) ;
            *(volatile DiReal32 *)(pl_map_base + offset) = value;
        }
    else if(DSP_PHY_ADDR <= addr && addr <= DSP_PHY_ADDR + DSP_SIZE)
        {
            offset = addr - DSP_PHY_ADDR ;
            *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + offset) = value;
        }
    else if(DETECTOR_PHY_ADDR <= addr && addr <= DETECTOR_PHY_ADDR + DETECTOR_SIZE)
        {
            offset = addr - DETECTOR_PHY_ADDR ;
            *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + offset) = value ;

        }
	else if(PARAM_PHY_ADDR <= addr && addr <= PARAM_PHY_ADDR + PARAM_SIZE)
        {
            offset = addr - PARAM_PHY_ADDR ;
            *(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + offset) = value;
        }
	else if(CONFIG_PHY_ADDR <= addr && addr <= CONFIG_PHY_ADDR + CONFIG_SIZE)
		{
			offset = addr - CONFIG_PHY_ADDR ;
			*(volatile DiReal32 *)(pl_map_base + CPLD_SIZE + DSP_SIZE + DETECTOR_SIZE + PARAM_SIZE + offset) = value;
		}
    else
        {
            return -1;
