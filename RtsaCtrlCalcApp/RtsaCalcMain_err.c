            ...
            for(j = 1;j < bitFreqList.count;j++)
                {
                    ...
                    if(!iflag || rerefset)
                        {
                            ...
                            segindex = RfChannelChoose(bitdevHandle,*(preflsit + ig));
                            printf("=======just once ref index is %d=========\n",segindex);
                    ...
                    if(what >= PRCEISE)
                        {
                            inow = -1;
                            double temperrfreq = 0.;
                            if(inow != ilast)
                                {
                                    for(e = 4;e >=1;e--)
                                        {
                                            temperrfreq = (long long)(*(bitFreqList.param + j)) - 50000000 - 10000000. * e;
                                            itemprerefset = RfChannelChoose(bitdevHandle,temperrfreq);
                                            printf("front oriindex is%d,reref index is %d\n",segindex,itemprerefset);
                                            if(temperrfreq >= 50000000.)
                                                {
                                                    if(itemprerefset != segindex)
                                                        {
                                                            rerefset = 1;
                                                            //do ref again
                                                            for(m = 0;m < bitFreqList.count;m++)
                                                                {
                                                                    if(*(bitFreqList.param + m) >= *(preflsit + itemprerefset - 1))
                                                                        {
                                                                            break;
                                                                        }
                                                                }
                                                            printf("preamp:0,ref freq %lf,posi is %d\n",*(preflsit + itemprerefset - 1),m);
                                                            SetSourOutputPower((maplist + n * bitFreqList.count + m)->ampdB);
                                                            SetSourceOutFreq(*(preflsit + itemprerefset - 1));
                                                            SetRtSaFpgaStop(bitdevHandle);
                                                            RfChannelChoose(bitdevHandle,*(preflsit + itemprerefset - 1));
                                                            FirstLoFreqCtrl(bitdevHandle,*(preflsit + itemprerefset - 1));
                                                            SetRtSaFpgaStart(bitdevHandle);
                                                            usleep(1000000);
                                                    ...
                                                    //==
                                                    SetSourOutputPower((maplist + n * bitFreqList.count + j - 1)->ampdB);
                                                    SetSourceOutFreq(temperrfreq);
                                                    SetRtSaFpgaStop(bitdevHandle);
                                                    RfChannelChoose(bitdevHandle,temperrfreq);
                                                    FirstLoFreqCtrl(bitdevHandle,temperrfreq);
                                                    SetRtSaFpgaStart(bitdevHandle);
                                                    usleep(1000000);
                                                    GetCtrlInHand(sweepHandle);
                                                    //usleep(100000);
                            ...
                            for(m = 0;m < bitFreqList.count;m++)
                                {
                                    if(*(bitFreqList.param + m) >= *(preflsit + ig - 1))
                                        {
                                            break;
                                        }
                                }
                            SetSourOutputPower((maplist + n * bitFreqList.count + m)->ampdB);
                            printf("preamp:0,ref freq %lf,posi is %d\n",*(preflsit + ig - 1),m);
                            SetSourceOutFreq(*(preflsit + ig - 1));
                            SetRtSaFpgaStop(bitdevHandle);
                            RfChannelChoose(bitdevHandle,*(preflsit + ig - 1));
                            FirstLoFreqCtrl(bitdevHandle,*(preflsit + ig - 1));
                            SetRtSaFpgaStart(bitdevHandle);
                            usleep(1000000);
                            ...
                            //when error point is not end point
                            if(IsEndfreqPoint(*(bitFreqList.param + j)) < 0)
                                {
                                    for(e = 1;e <= 4;e++)
                                        {
                                            temperrfreq = (long long)(*(bitFreqList.param + j)) + 50000000 + 10000000. * e;
                                            itemprerefset = RfChannelChoose(bitdevHandle,temperrfreq);
                                            printf("back oriindex is%d,reref index is %d\n",segindex,itemprerefset);
                                            if(temperrfreq < 43050000000)
                                                {
                                                    if(itemprerefset != segindex)
                                                        {
                                                            rerefset = 1;
            ...
            for(j = 1;j < bitFreqList.count;j++)
                {
                    ...
                    for(m = 0;m < BUFFERSIZE;m++)
                        {
                            //for solving cross cutting problems
                            if(what < fabs ((fabs(acqBuffer[m] - glotemp.ampbuf[m])) - fabs(refdeta)))
                                {
                                    what = fabs ((fabs(acqBuffer[m] - glotemp.ampbuf[m])) - fabs(refdeta));
                                    in = m;
                    ...
                    if(what >= PRCEISE)
                        {
                            ...
                            if(inow != ilast)
                                {
                                    for(e = 4;e >=1;e--)
                                        {
                                            temperrfreq = *(bitFreqList.param + j) - 50000000 - 10000000. * e;
                                            itemprerefset = RfChannelChoose(bitdevHandle,temperrfreq);
                                            printf("front oriindex is%d,reref index is %d\n",segindex,itemprerefset);
                                            if(temperrfreq >= 50000000.)
                                                {
                                                    ...
                                                    SetSourOutputPower((maplist + nstartIndex + i * bitFreqList.count + j - 1)->ampdB);
                                                    SetSourceOutFreq(temperrfreq);
                                                    SetRtSaFpgaStop(bitdevHandle);
                                                    RfChannelChoose(bitdevHandle,temperrfreq);
                                                    FirstLoFreqCtrl(bitdevHandle,temperrfreq);
                            ...
                            SetSourceOutFreq(*(preflsit + ig - 1));
                            SetRtSaFpgaStop(bitdevHandle);
                            segindex = RfChannelChoose(bitdevHandle,*(preflsit + ig - 1));
                            FirstLoFreqCtrl(bitdevHandle,*(preflsit + ig - 1));
                            SetRtSaFpgaStart(bitdevHandle);
                            usleep(1000000);
                            GetCtrlInHand(sweepHandle);
