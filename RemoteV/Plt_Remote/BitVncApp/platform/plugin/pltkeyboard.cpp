#include "pltkeyboard.h"
#include "ui_pltkeyboard.h"

pltKeyBoard::pltKeyBoard(QWidget *parent) :
    QDialog(parent),
    mParent(parent),
    ui(new Ui::pltKeyBoard)
{
    ui->setupUi(this);
...
void pltKeyBoard::init()
{
    if(mCapLockOn) CapslockChange();
    if(mShiftFlag) ShiftChange();
}
pltKeyBoard::~pltKeyBoard()
{
    delete ui;
}

void pltKeyBoard::initKeyBoard()
{
    qint32 btnCnt;
...
void pltKeyBoard::keyPressEvent(QKeyEvent *event)
{
...
void pltKeyBoard::CapslockChange()
{
...
void pltKeyBoard::on_pb_cancel_clicked()
{
    this->close();
}
