...
{
    Q_UNUSED(event)
    gpixmap = gpixmap.scaled(width(),height(),Qt::IgnoreAspectRatio,Qt::FastTransformation);
...
/**
* Mouse press event and processing contents.
...
//    if(gbitbuffer.gtracenum < this->width())
    if(this->width() > gbitbuffer.gtracenum)
...
void BitView:: renderSiglines(QPainter* pPainter)
{
    ...
    for(i  = 0;i < SUPPORTTRACENUM;i++)
    {
        ...
        {
            if(gctrlmana->gtracetype[i] != BIT_TRACE_OFF)
            {
                if (BITTR1 == i)
                {
...
void BitView::showTopPart()
{
    ...
    painter.drawRect(LEFTDIS,2*LINEREEOR,gusedsize.gpicwidth,TOPDIS - BITWORDHSET-2*LINEREEOR);
    renderTopPart(&painter);
    return;
}
...
void BitView::showBottomPart()
{
    ...
    QBrush mbrush(mPenColor.background);
    painter.setBrush(mbrush);
    painter.drawRect(LEFTDIS,TOPDIS + gusedsize.gpicheight + 2*LINEREEOR,gusedsize.gpicwidth,TOPDIS - BITWORDHSET-2*LINEREEOR);
    renderBottomPart(&painter);
...
void BitView::renderTopPart(QPainter *pPainter)
{
    ...
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT, Qt::AlignCenter, gwordset.gtscstr);

    //topfirstleft
    mx = LEFTDIS + HALFLINEREEOR;
    my = (LINEREEOR << 2) + HALFLINEREEOR;
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT, Qt::AlignLeft, gwordset.gtflstr);

    //topsecleft
    my = (LINEREEOR << 1) + WORDHEIGHT;
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT, Qt::AlignLeft, gwordset.gtslstr);
    return;
}
...
void BitView::renderBottomPart(QPainter *pPainter)
{
    ...
    pPainter->drawText(mx, my, TITLEWIDTH + 30, WORDHEIGHT, Qt::AlignLeft, gwordset.gbflstr);

    //bottomsecleft
    my = TOPDIS + gusedsize.gpicheight  + WORDHEIGHT - LINEREEOR;
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT + 30, Qt::AlignLeft, gwordset.gbslstr);

    //bottomfirstright
    mx = LEFTDIS + gusedsize.gpicwidth - TITLEWIDTH;
    my = TOPDIS + gusedsize.gpicheight + LINEREEOR;
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT, Qt::AlignRight, gwordset.gbfrstr);

    //bottomsectright
    my = TOPDIS + gusedsize.gpicheight  + WORDHEIGHT - LINEREEOR;
    pPainter->drawText(mx, my, TITLEWIDTH, WORDHEIGHT, Qt::AlignRight, gwordset.gbsrstr);
    return;
}
...
void BitView::renderBoaderLines(QPen *mpen,QPainter* mpainter)
{
    ...
    mpainter->drawLine(QPointF(LINEREEOR,LINEREEOR),QPointF(this->width()-LINEREEOR,LINEREEOR));
    mpainter->drawLine(QPointF(this->width()-LINEREEOR,LINEREEOR),QPointF(this->width()-LINEREEOR,this->height()-LINEREEOR ));
    mpainter->drawLine(QPointF(this->width()-LINEREEOR ,this->height()-LINEREEOR),QPointF(LINEREEOR,this->height()-LINEREEOR));
    mpainter->drawLine(QPointF(LINEREEOR,this->height() - LINEREEOR),QPointF(LINEREEOR,LINEREEOR));
...
void BitView::drawSpectrogramSmallPix(QPainter *painter)
{
    ...
    for(quint32 i = 0 ; i < gbitbuffer.gtracenum/* - 1*/; i++)
    {
        ...
        //! Albert Delete 1017. drawpointsX is double. no equal occasion.
        //! when drawpointsX use int, should use equal compare
        if(gbitbuffer.gtracenum > nWidthSmall)
        {
...
void BitView::RecalSpectrogramBigPointX()
{
    ...
    qreal tempWidth = gusedsize.gpicwidth;
    qint32 dtemp = 0.0;
...
qreal BitView::freqIndexToFreq(qint32 index)
{
    ...
    if(index > gbitbuffer.gtracenum - 1)
    {
...
void BitView::markerMove(qint32 type)
{
    ...
    case BIT_MARKER_MOVE_LEFT_MACRO:
        if(gctrlmana->gMarkerType[gctrlmana->gMarkerSelect] == BIT_NORMAL)
        {
            qint32 a = gctrlmana->gMarkerNormalIndex[gctrlmana->gMarkerSelect];
            if((gctrlmana->gMarkerNormalIndex[gctrlmana->gMarkerSelect] >= (gbitbuffer.gtracenum-1)/10))
            {
    ...
    case BIT_MARKER_MOVE_RIGHT_MICRO:
        if(gctrlmana->gMarkerType[gctrlmana->gMarkerSelect] == BIT_NORMAL)
        {
            if((gctrlmana->gMarkerNormalIndex[gctrlmana->gMarkerSelect] + 1) < (gbitbuffer.gtracenum-1))
            {
                ++gctrlmana->gMarkerNormalIndex[gctrlmana->gMarkerSelect];
                gctrlmana->gMarkerNormalFreq[gctrlmana->gMarkerSelect] = freqIndexToFreq(gctrlmana->gMarkerNormalIndex[gctrlmana->gMarkerSelect]);
            }
        ...
        {
            if((gctrlmana->gMarkerDeltaIndex[gctrlmana->gMarkerSelect] + 1) <= (gbitbuffer.gtracenum-1))
            {
                ++gctrlmana->gMarkerDeltaIndex[gctrlmana->gMarkerSelect];
                gctrlmana->gMarkerDeltaFreq[gctrlmana->gMarkerSelect] = freqIndexToFreq(gctrlmana->gMarkerDeltaIndex[gctrlmana->gMarkerSelect]);
            }
...
//            refVal = ChangedBmToUV(mMarkerRefAmpt);
            ...
            str = str + QString(" (V/V)");
//            ChangeValueToVoltStr(strAmp,deltaVal);
//            dbmToVoltCalculatorAndDisplayFunc(strAmp, value);
...
//            refVal = ChangedBmToMW(mMarkerRefAmpt);
            ...
            str = QString::number(deltaVal,'f',2);
            str = str + QString(" (W/W)");
//            ChangeValueToWattStr(strAmp,deltaVal);
...
//            {
//                gctrlmana->gMarkerDeltaIndex[i] = 0;
//                gctrlmana->gMarkerDeltaFreq[i] = gfreqinfo.gstartfreq;
//            }else if(gctrlmana->gMarkerDeltaFreq[i] > gfreqinfo.gstopfreq)
//            {
//                gctrlmana->gMarkerDeltaIndex[i] = gbitbuffer.gtracenum-1;
//                gctrlmana->gMarkerDeltaFreq[i] = gfreqinfo.gstopfreq;
//            }
...
//                if(gctrlmana->gMarkerDeltaFreq[i] < gfreqinfo.gstartfreq || gctrlmana->gMarkerDeltaFreq[i] > gfreqinfo.gstopfreq)
//                {
//                    gctrlmana->gMarkerDeltaIndex[i]= gbitbuffer.gtracenum/2;
//                    gctrlmana->gMarkerDeltaFreq[i] = gfreqinfo.gcenterfreq;
//                }
...
//                    ChangeValueToVoltStr(strAmp,deltaVolt);
                    sprintf(strAmp, "%.2f (V/V)", deltaVolt/* + dBuVFix*/);
...
//                    refWatt = ChangedBmToMW(mMarkerRefAmpt);
                    ...
                    ChangeValueToWattStr(strAmp,deltaWatt);
...
void BitView::setdisplaymode(qint32 displaymode)
{
    ...
    case BIT_DEFAULTMODE:
        mlimitTableWidget->setStyleSheet("border-color: rgb(255, 0, 0);selection-background-color: rgb(200, 200, 50);color: rgb(255, 255, 255);background-color: rgb(100,100,100);");
        produceColorBar(BIT_DEFAULTMODE);
        break;
    case BIT_OUTDOORMODE:
        mlimitTableWidget->setStyleSheet("border-color: rgb(255, 0, 0);selection-background-color: rgb(200, 100, 50);color: rgb(255, 255, 255);background-color: rgb(200,100,100);");
        produceColorBar(BIT_OUTDOORMODE);
        break;
    case BIT_NIGHTMODE:
        mlimitTableWidget->setStyleSheet("border-color: rgb(255, 0, 0);selection-background-color: rgb(100, 200, 50);color: rgb(255, 255, 255);background-color: rgb(100,200,100);");
...
#if 0
    qDebug() << "aaaa" << gctrlmana->uFmtRGB[0] << gctrlmana->uFmtRGB[4] << gctrlmana->uFmtRGB[8];
    qDebug() << "bbbb" << gctrlmana->uFmtRGB[1] << gctrlmana->uFmtRGB[5] << gctrlmana->uFmtRGB[9];
    qDebug() << "cccc" << gctrlmana->uFmtRGB[2] << gctrlmana->uFmtRGB[6] << gctrlmana->uFmtRGB[10];
    qDebug() << "dddd" << gctrlmana->uFmtRGB[3] << gctrlmana->uFmtRGB[7] << gctrlmana->uFmtRGB[11];
...
//    myPolygon.append(QPointF(LEFTDIS,TOPDIS));
    ...
    QPainterPath myPath;
    myPath.addPolygon(gctrlmana->gFmtPolygon);
    oldBrush = painter->brush();
...
void BitView::FmtXAxisCalculatorFunc(qreal *Freq, quint32 datanum)
{
    ...
    for(qint32 i = 0; i < datanum; i++)
    {
        if(Freq[i] < gfreqinfo.gstartfreq / BV_MHZ)
        {
            gctrlmana->gFmtPointsX[i] = LEFTDIS;
        }
        else
        {
            gctrlmana->gFmtPointsX[i] = widDiv *  (Freq[i] * BV_MHZ - gfreqinfo.gstartfreq) / (gfreqinfo.gstopfreq - gfreqinfo.gstartfreq) + LEFTDIS;
        }
    }
}
/**
...
void BitView::renderVideoTriggerLevel(QPainter *painter)
{
    ...
    {
        painter->drawLine(QPointF(gctrlmana->gVideoPointsX[k-1],gctrlmana->gVideoPointsY[k-1]),QPointF(gctrlmana->gVideoPointsX[k],gctrlmana->gVideoPointsY[k]));
    }
    return;
}
/**
