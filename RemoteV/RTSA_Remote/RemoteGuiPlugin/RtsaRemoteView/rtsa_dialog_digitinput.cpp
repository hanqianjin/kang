 ...
 * @since 2018-12-25
*/
void RtSa_Dialog_Digitinput::setInterface(Rtsa_Remote_Interface *RtsaRemoteInterface)
{
...
void RtSa_Dialog_Digitinput::showAmptUnitIndex(qint32 index)
{
    return;
}
/**
...
void RtSa_Dialog_Digitinput::initDisplay(qint32 index)
{
    ...
    qreal   valueDelta;
    qint32  intGab;
    VARIABLE_IS_NOT_USED qint32 minusFlag = 0;
    qint32 curMarkerID = RemoteInterfaceObj->getCurMarkerID();
    //Johnson 0630
    ...
    case RTSA_INPUT_FREQ_CENTER:
        ...
        unitNo = RemoteInterfaceObj->freqUnitFindProc(value);
...
//        showFreqUnit(false);
        ...
        unitNo = RemoteInterfaceObj->getAmptUint();
...
//        showFreqUnit(false);
        ...
        unitNo = RemoteInterfaceObj->timeUnitFindProc(value);
...
//        showAmptUnit(false);
        showFreqUnit(true);
        value = RemoteInterfaceObj->getMarkerRefFreq(curMarkerID);
        strGab = RemoteInterfaceObj->getFreqAdjustStr(value);
        unitNo = RemoteInterfaceObj->freqUnitFindProc(value);
    ...
    case RTSA_INPUT_DELTA_MARKER_EDIT:
        showFreqUnit(true);
//        showAmptUnit(false);
        valueDelta = RemoteInterfaceObj->getMarkerRefFreq(curMarkerID);
        value = RemoteInterfaceObj->getMarkerDeltaFreq(curMarkerID);
        value -= valueDelta;
        strGab = RemoteInterfaceObj->getFreqAdjustStr(value);
        unitNo = RemoteInterfaceObj->freqUnitFindProc(value);
...
//        showFreqUnit(false);
        showAmptUnit(false);
        value = RemoteInterfaceObj->getAvgNum(RemoteInterfaceObj->getCurTrace());
...
//        showFreqUnit(false);
        ...
        strUnit =QString("(");
        if(m_usr_variable->gMeterParam.gConfig_FreqSpan.channelType == RTSA_UPLINK)
        {
            value = m_usr_variable->gCHStandard->at(m_usr_variable->gMeterParam.gConfig_FreqSpan.channelSelID).Up.Min;
            strUnit = strUnit + QString::number(value);
            strUnit = strUnit + QString("-");
            value = m_usr_variable->gCHStandard->at(m_usr_variable->gMeterParam.gConfig_FreqSpan.channelSelID).Up.Max;
            strUnit = strUnit + QString::number(value);
        }
        else
        {
            value = m_usr_variable->gCHStandard->at(m_usr_variable->gMeterParam.gConfig_FreqSpan.channelSelID).Down.Min;
            strUnit = strUnit + QString::number(value);
            strUnit = strUnit + QString("-");
            value = m_usr_variable->gCHStandard->at(m_usr_variable->gMeterParam.gConfig_FreqSpan.channelSelID).Down.Max;
            strUnit = strUnit + QString::number(value);
        }
        strUnit = strUnit + QString(")");
    ...
    case RTSA_INPUT_LIMIT_FREQ:
        intGab = RemoteInterfaceObj->GetCurLimitDataIndex();
        value = RemoteInterfaceObj->getLimitCurPointFreq(intGab,RemoteInterfaceObj->getCurLimit()) * RTSA_MHZ;
        strGab = RemoteInterfaceObj->getFreqAdjustStr(value);
        unitNo = RemoteInterfaceObj->freqUnitFindProc(value);
...
//        showAmptUnit(false);
    ...
    case RTSA_INPUT_LIMIT_AMPT:
        intGab = RemoteInterfaceObj->GetCurLimitDataIndex();
        value = RemoteInterfaceObj->getLimitCurPointAmpt(intGab,RemoteInterfaceObj->getCurLimit());
...
//        showFreqUnit(true);
    ...
    case RTSA_INPUT_LIMIT_SELECT_INDEX:
        showAmptUnit(false);
        value = RemoteInterfaceObj->GetCurLimitDataIndex() + 1;
//        value = mControl->getAvgNum(mControl->getCurTrace());
    ...
    case RTSA_INPUT_FMT_FREQ:
        intGab = RemoteInterfaceObj->getCurFmtDataIndex();
        value = RemoteInterfaceObj->getCurFmtFreqData(intGab) * RTSA_MHZ;
        strGab = RemoteInterfaceObj->getFreqAdjustStr(value);
        unitNo = RemoteInterfaceObj->freqUnitFindProc(value);
...
//        showAmptUnit(false);
    ...
    case RTSA_INPUT_FMT_AMPT:
        intGab = RemoteInterfaceObj->getCurFmtDataIndex();
        value = RemoteInterfaceObj->getCurFmtAmptData(intGab);
        strGab.sprintf("%.2f dBm", value);
        showAmptUnit(true);
        ui->amptBtnUnit1->hide();
        ui->amptBtnUnit2->hide();
...
//        showFreqUnit(true);
    ...
    case RTSA_INPUT_FMT_SELECT_INDEX:
        showAmptUnit(false);
        value = RemoteInterfaceObj->getCurFmtDataIndex() + 1;
        strGab = QString::number(value,'f',0) + QString("");
        ui->lblTitle->setText("Current Row Index ");
        ui->amptBtnUnit4->show();
        ui->amptBtnUnit4->setText("Enter");
    ...
    case RTSA_INPUT_TRIGGER_DELAY_VALUE:
        ...
        value = RemoteInterfaceObj->getTriggerDelay(intGab);
        strGab = RemoteInterfaceObj->getAdjustTimeStr(value);
        unitNo = RemoteInterfaceObj->timeUnitFindProc(value);
    ...
    case RTSA_INPUT_TRIGGER_LEVEL:
        intGab = RemoteInterfaceObj->getTrigger();
        value = RemoteInterfaceObj->getTriggerLevel(intGab);
        strGab.sprintf("%.2f dBm", value);
        showAmptUnit(true);
        ui->amptBtnUnit1->hide();
        ui->amptBtnUnit2->hide();
    ...
    case RTSA_INPUT_TRIGGER_HOLDTIME:
        ...
        ui->amptBtnUnit7->hide();
        intGab = RemoteInterfaceObj->getTrigger();
        value = RemoteInterfaceObj->getriggerHoldOffTime(intGab);
        strGab = RemoteInterfaceObj->getAdjustTimeStr(value);
        unitNo = RemoteInterfaceObj->timeUnitFindProc(value);
...
void RtSa_Dialog_Digitinput::getKnobValue(quint32 mode)
{
    ...
    switch (entryNo){
      ...
      case RTSA_INPUT_LIMIT_FREQ:
      case RTSA_INPUT_FMT_FREQ:
        strValue = getNewFreqDataStringValue(mode,RemoteInterfaceObj->getStepFreq(),unitNo);
...
//       break;
   ...
   case Qt::Key_F2:    //  +/-
       ...
       else
       {
           ...
           {
               if(RemoteInterfaceObj->getFreqOffset() <= 0.00001 && RemoteInterfaceObj->getFreqOffset() >= -0.00001)
               {
                   return;
               }
           }
...
//   int i = 0;
  ...
  if(entryNo == RTSA_INPUT_DELTA_MARKER_EDIT)
  {
      if(minusFlag == 1)
      {
          freq = RemoteInterfaceObj->getMarkerRefFreq(RemoteInterfaceObj->getCurMarkerID()) - freq;
      }
      else
      {
          freq = RemoteInterfaceObj->getMarkerRefFreq(RemoteInterfaceObj->getCurMarkerID()) + freq;
...
//      break;
  ...
  if ( m_usr_variable->gMenu ==  RTSA_MENU_SPECTRUM_ANALYZER) {
      switch (entryNo)  {
      case RTSA_INPUT_FREQ_CENTER:
          RemoteInterfaceObj->setCenterFreq(freq);
          break;
        case RTSA_INPUT_FREQ_START:
          RemoteInterfaceObj->setStartFreq(freq);
          break;
        case RTSA_INPUT_FREQ_STOP:
          RemoteInterfaceObj->setStopFreq(freq);
          break;
        case RTSA_INPUT_FREQ_STEP:
          RemoteInterfaceObj->setCFStep(freq);
          break;
        case RTSA_INPUT_FREQ_SPAN:
          RemoteInterfaceObj->setSpan(freq);
          break;
        case RTSA_INPUT_FREQ_OFFSET:
          RemoteInterfaceObj->setFreqOffset(freq);
          break;
        case RTSA_INPUT_MARKER_EDIT:
          RemoteInterfaceObj->setMarkerRefFreq(RemoteInterfaceObj->getCurMarkerID(),freq);
          break;
        case  RTSA_INPUT_DELTA_MARKER_EDIT:
          RemoteInterfaceObj->setMarkerDeltaFreq(RemoteInterfaceObj->getCurMarkerID(),freq);
          break;
      case RTSA_INPUT_LIMIT_FREQ:
          RemoteInterfaceObj->setLimitCurPointFreq(freq/RTSA_MHZ,RemoteInterfaceObj->GetCurLimitDataIndex(),RemoteInterfaceObj->getCurLimit());
//          mControl->setLimitFreq(freq);
          break;
      case RTSA_INPUT_FMT_FREQ:
          RemoteInterfaceObj->setCurFmtFreqData(freq/RTSA_MHZ,RemoteInterfaceObj->getCurFmtDataIndex());
          break;
...
//  }
  ...
  {
  case RTSA_INPUT_PERSISTENCE_TIME:
      RemoteInterfaceObj->setPersistenceTime(_time);
      break;
  case RTSA_INPUT_ACQ_TIME:
      RemoteInterfaceObj->setAcqTime(_time);
      break;
  case RTSA_INPUT_TRIGGER_DELAY_VALUE:
      RemoteInterfaceObj->setTriggerDelay(RemoteInterfaceObj->getTrigger(),_time);
      break;
  case RTSA_INPUT_TRIGGER_PERTIMER:
      RemoteInterfaceObj->setTriggerPeriodTime(_time);
      break;
  case RTSA_INPUT_TRIGGER_HOLDTIME:
      RemoteInterfaceObj->settriggerHoldOffTime(RemoteInterfaceObj->getTrigger(),_time);
      break;
    case RTSA_INPUT_SWEEP_TIME:
...
//    qint32 AmptUnit;
    ...
    case RTSA_INPUT_REF_LEVEL: //Johnson 0918
        RemoteInterfaceObj->setRefLevel(value);
        break;
    case RTSA_INPUT_REF_OFFSET: //Johnson 0918
        RemoteInterfaceObj->setRefOffset(value);
        break;
    case RTSA_INPUT_POWERSENSOR_EXT_OFFSET:
        ...
        break;
    case RTSA_INPUT_ATTEN:
        RemoteInterfaceObj->setAttenVal(value);
        break;
    case RTSA_INPUT_SCALEDIV:
        RemoteInterfaceObj->setScaleDiv(value);
        break;
    case RTSA_LIMIT_EDGE:
        RemoteInterfaceObj->setMargin(RemoteInterfaceObj->getCurLimit(),value);
        break;
    case RTSA_INPUT_MARGIN:
        RemoteInterfaceObj->setMargin(RemoteInterfaceObj->getCurLimit(),value);
        break;
    case RTSA_INPUT_AVERAGE_COUNT:
        RemoteInterfaceObj->setAvgNum(RemoteInterfaceObj->getCurTrace(),value);
        break;
    case RTSA_INPUT_LIMIT_SELECT_INDEX:
        RemoteInterfaceObj->SetCurLimitDataIndex(value - 1);
        break;
    case RTSA_INPUT_FREQ_CHANNEL_NUM:
        RemoteInterfaceObj->setChannelNumber(value);
        break;
    case RTSA_INPUT_LIMIT_AMPT:
        RemoteInterfaceObj->setLimitCurPointAmpt(value,RemoteInterfaceObj->GetCurLimitDataIndex(),RemoteInterfaceObj->getCurLimit());
        break;
    case RTSA_INPUT_FMT_AMPT:
        RemoteInterfaceObj->setCurFmtAmptData(value,RemoteInterfaceObj->getCurFmtDataIndex());
        break;
    case RTSA_INPUT_TRIGGER_LEVEL:
        RemoteInterfaceObj->setTriggerLevel(RemoteInterfaceObj->getTrigger(),value);
        break;
    case RTSA_INPUT_FMT_SELECT_INDEX:
        RemoteInterfaceObj->setCurFmtDataIndex(value - 1);
        break;
...
//    qreal value = 0;
//    qint32 retNo = 0;
//    if(entryNo == RTSA_INPUT_HIGHEST_DENSITY || entryNo == RTSA_INPUT_LOWEST_DENSITY || entryNo == RTSA_INPUT_CURVE)
//    {
//        retNo = enterFunc();
//    }
//    else
...
int RtSa_Dialog_Digitinput::enterFunc()
{
    ...
    {
    case RTSA_INPUT_HIGHEST_DENSITY:
        RemoteInterfaceObj->setHighestDensity(value);
        break;
    case RTSA_INPUT_LOWEST_DENSITY:
        RemoteInterfaceObj->setLowestDensity(value);
        break;
    case RTSA_INPUT_CURVE:
        RemoteInterfaceObj->setCurve(value);
        break;
    }
    return 1;
}
