...
RtSa_Dialog_FmtTable::RtSa_Dialog_FmtTable(Rtsa_Remote_Interface *RtsaRemoteInterface, RtSa_Dialog_Digitinput *input, QWidget *parent) :
    QWidget(parent),
...
//            initTable();
        ...
        else
        {
            ui->tableWidget->insertRow(0);
    //        ui->tableWidget->setItem(0,0,new QTableWidgetItem(QString("%1").arg(0)));
            strValue  = QString::number(RemoteInterfaceObj->getStartFreq()/1000000,'f',2);
            ui->tableWidget->setItem(0,0,new QTableWidgetItem(strValue));
            ui->tableWidget->setItem(0,1,new QTableWidgetItem(QString("-10.00")));
        //    ui->tableWidget->sortAllItems();
...
//    row += 1;
    RemoteInterfaceObj->setFmtDataNum(row);
//    ui->tableWidget->resetAllData();
//    ui->tableWidget->resortAllData();
    RemoteInterfaceObj->setFmtFreqData(ui->tableWidget->getLimitFreq(),row);
    RemoteInterfaceObj->setFmtAmptData(ui->tableWidget->getLimitAmpt(),row);
//    initTable();
    ui->tableWidget->setCurrentCell(0,0);
    RemoteInterfaceObj->setCurFmtDataIndex(0);
    refreshCurRowIndex();
...
//    qint32 a = ui->tableWidget->rowCount();
    ...
    qint32 tatolnum = RemoteInterfaceObj->getFmtDataNum();
    qreal freq = ui->tableWidget->item(0,0)->text().toDouble();
    if((freq) <= 0.02 && (freq) >= -0.02)
    {
        ui->tableWidget->removeRow(index);
        RemoteInterfaceObj->setFmtDataNum(tatolnum - 1);
        return;
    }
    ui->tableWidget->removeRow(index);
    RemoteInterfaceObj->setFmtDataNum(tatolnum - 1);
//    ui->tableWidget->sortByColumn(0,Qt::AscendingOrder);
    ui->tableWidget->resetAllData();
//    ui->tableWidget->resortAllData();
    RemoteInterfaceObj->setFmtFreqData(ui->tableWidget->getLimitFreq() ,tatolnum -1);
    RemoteInterfaceObj->setFmtAmptData(ui->tableWidget->getLimitAmpt(),tatolnum -1);
...
void RtSa_Dialog_FmtTable::on_tableWidget_cellClicked(int row, int column)
{
    ...
    RemoteInterfaceObj->setCurFmtDataIndex(row);
...
//    mSelectInputID = index;
    ...
    mInput->setProperty("status", rtsa_qssMode[RemoteInterfaceObj->GetDisplayMode()]);
...
//    qint32 index;
    ...
    size = RemoteInterfaceObj->getFmtDataNum();
    qreal* freq = RemoteInterfaceObj->getFMTFreqData();
    qreal* ampt = RemoteInterfaceObj->getFMTAmplitudeData();
...
//    ui->tableWidget->judgeFmtTableContent(ui->tableWidget,mControl->getStartFreq(),mControl->getStopFreq());
    ...
    qreal   value;
    qint32 cnt = 0;
    cnt  = RemoteInterfaceObj->getCurFmtDataIndex();
    switch (index)
    {
    case RTSA_INPUT_FMT_FREQ:
        initTable();
        value = RemoteInterfaceObj->getCurFmtFreqData(cnt);
        strGab = QString::number((qreal) value,'f',2);
        ui->tableWidget->setItem(cnt,0,new QTableWidgetItem(strGab));
        refreshFmtData();

        ui->tableWidget->setCurrentCell(RemoteInterfaceObj->getCurFmtDataIndex(),0);
        break;
    case RTSA_INPUT_FMT_AMPT:
        initTable();
        value = RemoteInterfaceObj->getCurFmtAmptData(RemoteInterfaceObj->getCurFmtDataIndex());
...
{
    qint32 index = RemoteInterfaceObj->getCurFmtDataIndex() + 1;
...
{
    ui->tableWidget->judgeFmtTableContent(ui->tableWidget,RemoteInterfaceObj->getStartFreq(),RemoteInterfaceObj->getStopFreq());
    ui->tableWidget->resortAllData();
    qint32 row = ui->tableWidget->rowCount();
    RemoteInterfaceObj->setFmtDataNum(row);
    mLimitFreq = ui->tableWidget->getLimitFreq();
    mLimitAmpt = ui->tableWidget->getLimitAmpt();
    RemoteInterfaceObj->setFmtFreqData(mLimitFreq,row);
    RemoteInterfaceObj->setFmtAmptData(mLimitAmpt,row);
    return;
}
