...
RtSa_Dialog_AllSetup::RtSa_Dialog_AllSetup(Rtsa_Remote_Interface *RtsaRemoteInterface, RtSa_Dialog_Digitinput *input, QWidget *parent) :
    QWidget(parent),
...
//    connect(mInputDialog,SIGNAL(ClockWiseDown(QKeyEvent*)),this,SLOT(DigitalKeyRespond(QKeyEvent*)));
    this->hide();
  //  mUserVariable = RemoteInterfaceObj->getGlobalUserVariable();????
...
void RtSa_Dialog_AllSetup::initMap()
{
    ...
    mWidgetList.insert(RTSA_INPUT_FREQ_STEP,ui->Freq_Step);
...
void RtSa_Dialog_AllSetup::showInputDialog(qint32 index)
{
    ...
    else
    {
        ...
        mInputDialog->setInterface(RemoteInterfaceObj);
...
//    qreal freq;
    ...
    case RTSA_INPUT_FREQ_STEP:
        strGab = RemoteInterfaceObj->getFreqStr(RTSA_STEP_FREQ);
...
void RtSa_Dialog_AllSetup::dbmToVoltCalculatorAndDisplayFunc(char *label, double _level)
{
    return;
}
void RtSa_Dialog_AllSetup::dbmToWattsCalculatorAndDisplayFunc(char *label, double _level)
{
    return;
}
QString RtSa_Dialog_AllSetup::amptTransformToQstring(qint32 unit, qreal value)
{
    return "";
}
...
void RtSa_Dialog_AllSetup::refreshTrace()
{
    qint32 intGab;
    QString strGab;
    qint32 curTraceIndex = RemoteInterfaceObj->getCurTrace();

    ui->value_Trace->setCurrentIndex(curTraceIndex);

    intGab = RemoteInterfaceObj->getTraceType(curTraceIndex);
...
//    {
//    }
    intGab = RemoteInterfaceObj->getAvgNum(curTraceIndex);
...
//    qreal valGab;
    ...
    blGab = RemoteInterfaceObj->isMarkerOn(curMarkerID);
    if(blGab)
    {
    ...
    intGab = RemoteInterfaceObj->getMarkerType(curMarkerID);
    if(intGab == RTSA_REF)
    {
    ...
    blGab = RemoteInterfaceObj->IsMarkerNoiseOn(curMarkerID);
    if(blGab)
    {
...
{
    QString strGab;
    strGab = RemoteInterfaceObj->getFreqStr(RTSA_CENTER_FREQ);
    ui->value_CenterFreq->setText(strGab);
    strGab = RemoteInterfaceObj->getFreqStr(RTSA_START_FREQ);
    ui->value_StartFreq->setText(strGab);
    strGab = RemoteInterfaceObj->getFreqStr(RTSA_STOP_FREQ);
    ui->value_StopFreq->setText(strGab);
    strGab = RemoteInterfaceObj->getFreqStr(RTSA_SPAN_FREQ);
...
void RtSa_Dialog_AllSetup::refreshAmpt()
{
    ...
    valGab = RemoteInterfaceObj->getScaleDiv();
    strGab = QString::number(valGab);
    ui->value_Scale->setText(strGab);

    valGab = RemoteInterfaceObj->getAttenVal();
    strGab = QString::number(valGab);
    ui->value_AttenValue->setText(strGab);

    valGab = RemoteInterfaceObj->getRefOffset();
    strGab = QString::number(valGab);
    ui->value_RefOffset->setText(strGab);

    blGab = RemoteInterfaceObj->IsAttenAuto();
    if(blGab)
    {
    ...
    blGab = RemoteInterfaceObj->IsPreAmpOn();
    if(blGab)
    {
...
void RtSa_Dialog_AllSetup::refreshSweep()
{
    ...
    valGab = RemoteInterfaceObj->getAcqTime();
    strGab = RemoteInterfaceObj->getAdjustTimeStr(valGab);
    ui->value_Acqtime->setText(strGab);

    valGab = RemoteInterfaceObj->getPersistenceTime();
    strGab = RemoteInterfaceObj->getAdjustTimeStr(valGab);
    ui->value_Persistence->setText(strGab);

    intGab = RemoteInterfaceObj->getHighestDensity();
    strGab = QString::number(intGab) + QString(" %");
    ui->value_HighDensity->setText(strGab);

    intGab = RemoteInterfaceObj->getLowestDensity();
    strGab = QString::number(intGab) + QString(" %");
    ui->value_LowDensity->setText(strGab);

    intGab = RemoteInterfaceObj->getCurve();
    strGab = QString::number(intGab) + QString(" %");
    ui->value_Curve->setText(strGab);

    blGab = RemoteInterfaceObj->IsSingleSweep();
    if(blGab)
    {
    ...
    blGab = RemoteInterfaceObj->IsAcqTimeAuto();
    if(blGab)
    {
    ...
    blGab = RemoteInterfaceObj->IsPersistenceTimeInfinite();
    if(blGab)
    {
...
void RtSa_Dialog_AllSetup::refreshBW()
{
    ...
    {
        dbGab = RemoteInterfaceObj->getCurIndexBW(i);
        strGab = RemoteInterfaceObj->getFreqAdjustStr(dbGab);
        ui->value_RBW->setItemText(i,strGab);
    }
    intGab = RemoteInterfaceObj->getBWIndex();
    ui->value_RBW->setCurrentIndex(intGab);

    intGab = RemoteInterfaceObj->getFFTWindow();
    ui->value_VBW->setCurrentIndex(intGab);

    lbGab = RemoteInterfaceObj->IsBWAuto();
    if(lbGab)
    {
...
{
    RemoteInterfaceObj->setAmptUnit(index);
    refreshAmpt();
    return;
}
/**
...
{
    RemoteInterfaceObj->setSingleSweep(false);
    return;
}
/**
...
{
    RemoteInterfaceObj->setBWIndex(index);
    ui->RBW_Manual->setChecked(true);
    return;
}
/**
...
void RtSa_Dialog_AllSetup::on_value_VBW_currentIndexChanged(int index)
{
    ...
    qreal dbGab;
    QString strGab;
    RemoteInterfaceObj->setFFTWindow(index);
...
{
    RemoteInterfaceObj->setBWAuto(true);
    qint32 intGab = RemoteInterfaceObj->getBWIndex();
    ui->value_RBW->setCurrentIndex(intGab);
    ui->RBW_Auto->setChecked(true);
    return;
}
...
{
    RemoteInterfaceObj->setBWAuto(false);
    refreshBW();
    return;
}
/**
...
{
    RemoteInterfaceObj->setCurTrace(index);
    refreshTrace();
    return;
}
/**
...
{
    qint32 intGab = RemoteInterfaceObj->getCurTrace();
    qint32 traceType = RemoteInterfaceObj->getTraceType(intGab);
    ...
    if(checked)
    {
        ...
        {
           RemoteInterfaceObj->setTraceType(intGab,RTSA_CLEAR_WRITE);
        }
    }
    else
    {
        ...
        if(traceType != RTSA_TRACE_OFF)
        {
           RemoteInterfaceObj->setTraceType(intGab,RTSA_TRACE_OFF);
        }
    }
    return;
}
...
void RtSa_Dialog_AllSetup::on_btn_MarkerView_toggled(bool checked)
{
    ...
    RemoteInterfaceObj->setMarkerOn(curMarkerID,checked);
    refreshMarker();
    return;
}
/**
...
void RtSa_Dialog_AllSetup::on_btn_NoiseMarker_toggled(bool checked)
{
    ...
    {
        ui->btn_NoiseMarker->setText("On");
        RemoteInterfaceObj->setMarkerOn(curMarkerID,true);
    }
    else
    {
        ui->btn_NoiseMarker->setText("Off");
    }
    RemoteInterfaceObj->setMarkerNoiseOn(curMarkerID,checked);
    refreshMarker();
    return;
}
/**
...
void RtSa_Dialog_AllSetup::on_value_TraceType_currentIndexChanged(int index)
{
    ...
    qint32 nAvgCnt = 0;
    if(RemoteInterfaceObj->getTraceType(nCurTrace) == RTSA_TRACE_OFF)
        return;
//    mControl->setCurTrace(mCurSelTraceIndex);
    RemoteInterfaceObj->setTraceType(nCurTrace,index + 1);
    if(index >= (RTSA_AVERAGE - 1))
    {
        nAvgCnt = RemoteInterfaceObj->getAvgNum(nCurTrace);
...
{
    RemoteInterfaceObj->setTrigger(index + 1);
    return;
}
