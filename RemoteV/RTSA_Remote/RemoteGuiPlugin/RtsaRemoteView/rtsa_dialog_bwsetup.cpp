...
RtSa_Dialog_BWSetup::RtSa_Dialog_BWSetup(RtSa_User_Variable *UserVariable,Rtsa_Remote_Interface *RtsaRemoteInterface, QWidget *parent) :
    QWidget(parent),
...
void RtSa_Dialog_BWSetup::InitDisplay()
{
    ...
    mRBWGroup->button(RemoteInterfaceObj->getBWIndex())->setChecked(true);
    ...
    mAutoGroup->button(RemoteInterfaceObj->IsBWAuto())->setChecked(true);
...
{
    if(RemoteInterfaceObj->IsBWAuto())
    {
...
void RtSa_Dialog_BWSetup::InitRBWIndex()
{
    ...
    {
        qreal freq = RTSA_RbwManualList[globalUserVariable->gMeterParam.gConfig_BW.fftFilter][globalUserVariable->gRbwGrade][i];
        QString data = RemoteInterfaceObj->getFreqAdjustStr(freq,3);
        mRBWGroup->button(i)->setText(data);
    }
    mCurRBWIndex = RemoteInterfaceObj->getBWIndex();
...
{
    mCurFFTWindow = RemoteInterfaceObj->getFFTWindow();
...
{
    if(index == RemoteInterfaceObj->getBWIndex())
        return;
    if(RemoteInterfaceObj->getFFTWindow() == RTSA_RECTANGULAR && index < RTSA_RBW6)
    {
        RtSa_Dialog_Warning message("Notice","Can not change BW in Rectangular Filter!",this,RemoteInterfaceObj->GetDisplayMode());
...
{
    if(index == RemoteInterfaceObj->getFFTWindow())
        return;
...
{
    RemoteInterfaceObj->setBWAuto(index);
    InitRBWIndex();
    return;
}
