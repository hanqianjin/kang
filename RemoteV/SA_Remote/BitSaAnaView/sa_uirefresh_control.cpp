...
void sa_UIRefresh_Control::refreshPixmap()
{
    ...
    if(globalUserVariable->gTriggerMode == SA_TRIGGER_VIDEO || globalUserVariable->gTriggerMode == SA_TRIGGER_RF_BURST ||
       (globalUserVariable->gDrCtrlSource == SA_RFBURST_TRIGGER && globalUserVariable->gDrCtrlSwitch == SA_OPEN_DCSWEEP))
    {
        qreal trigLevel = 0.;
        if(globalUserVariable->gTriggerMode == SA_TRIGGER_VIDEO || globalUserVariable->gTriggerMode == SA_TRIGGER_RF_BURST)
        {
    ...
    if(globalUserVariable->gLvlEdgSelect == SA_EDGE_MODE && globalUserVariable->gGateViewStatus == SA_GATE_VIEW_OPEN &&
       globalUserVariable->gDrCtrlSwitch == SA_OPEN_DCSWEEP)
    {
...
void sa_UIRefresh_Control::drawGateView(QPainter *painter)
{
    ...
    painter->drawText(startXPos + 5,DRAW_GRID_START_Y + 50,QString("Gate Start"));
 ...
 */
void sa_UIRefresh_Control::drawChannelPower(QPainter *painter, qint32 chpw_x, qint32 chpw_w, qreal pixelStep, qint32 gridWidth)
{
    qreal dPowerValue = 0.;
    qint32 y;
    QString powerValue;
    ...
    dPowerValue = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + chpw_x,chpw_w,gridWidth,pixelStep);//Johnson 0801
    ...
    painter->fillRect(DRAW_GRID_START_X + chpw_x, y, chpw_w, globalUserVariable->gridYPos[10] - y + 1, colSteelBlue);
...
void sa_UIRefresh_Control::drawACPPower(QPainter *painter,qint32 chpw_x,qint32 chpw_w,qreal pixelStep,qint32 gridWidth)
{
    ...
    //Johnson 0801
    dPowerValue_ACPM = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + chpw_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_ACPL = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + acpl_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_ACPR = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + acpr_x,chpw_w,mPixmap.width(),pixelStep);
    ...
    //Johnson 0815 draw dbm value
    powerValue = QString::number(dPowerValue_ACPL,'f',2);
    ...
    //Johnson 0815 draw dbm value
    powerValue = QString::number(dPowerValue_ACPR,'f',2);
 ...
 * @author Johnson 190326
 */
void sa_UIRefresh_Control::drawMultiACPPower(QPainter *painter, qint32 chpw_x, qint32 chpw_w, qreal pixelStep, qint32 gridWidth)
{
    ...
    //Johnson 190325
    dPowerValue_MultiACPM  = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + chpw_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_MultiACPFL = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + multiacpFarl_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_MultiACPL  = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + multiacpl_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_MultiACPFR = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + multiacpFarr_x,chpw_w,mPixmap.width(),pixelStep);
    dPowerValue_MultiACPR  = psa_Measure_Control->calculatePower(DRAW_GRID_START_X + multiacpr_x,chpw_w,mPixmap.width(),pixelStep);
    ...
    painter->fillRect(DRAW_GRID_START_X + chpw_x, multiacp_y_M, chpw_w, globalUserVariable->gridYPos[10]- multiacp_y_M + 1, colSteelBlue);
    painter->fillRect(DRAW_GRID_START_X + multiacpFarl_x, multiacp_y_FarL, chpw_w, globalUserVariable->gridYPos[10]- multiacp_y_FarL + 1, colSteelBlue);
    painter->fillRect(DRAW_GRID_START_X + multiacpl_x, multiacp_y_L, chpw_w, globalUserVariable->gridYPos[10]- multiacp_y_L + 1, colSteelBlue);
    painter->fillRect(DRAW_GRID_START_X + multiacpFarr_x, multiacp_y_FarR, chpw_w, globalUserVariable->gridYPos[10]- multiacp_y_FarR + 1, colSteelBlue);
    painter->fillRect(DRAW_GRID_START_X + multiacpr_x, multiacp_y_R, chpw_w, globalUserVariable->gridYPos[10]- multiacp_y_R + 1, colSteelBlue);
    ...
    painter->setPen(Qt::white);
    if(multiacpFarl_x != 0) painter->drawLine(DRAW_GRID_START_X+multiacpFarl_x,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpFarl_x,globalUserVariable->gridYPos[10]);
    if(multiacpFarl_x+chpw_w != gridWidth) painter->drawLine(DRAW_GRID_START_X+multiacpFarl_x+chpw_w,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpFarl_x+chpw_w,globalUserVariable->gridYPos[10]);
    //Johnson 190326 draw arrow line
    painter->drawText(DRAW_GRID_START_X+multiacpFarl_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, "<");
    painter->drawLine(DRAW_GRID_START_X+multiacpFarl_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2, DRAW_GRID_START_X+chpw_w+multiacpFarl_x, DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2);
    painter->drawText(DRAW_GRID_START_X+multiacpFarl_x+chpw_w-fm.width(">") , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, ">");
    //Johnson 190326 draw dbm value
    powerValue = QString::number(dPowerValue_MultiACPFL,'f',2);
    painter->drawText(DRAW_GRID_START_X+multiacpFarl_x+chpw_w/2-fm.width(powerValue)/2,DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2 + 5,fm.width(powerValue),fm.height(),Qt::AlignLeft,powerValue);
    //********draw Far left MultiACP end*************

    //********draw left MultiACP begin***********
    painter->setPen(Qt::white);
    if(multiacpl_x != 0) painter->drawLine(DRAW_GRID_START_X+multiacpl_x,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpl_x,globalUserVariable->gridYPos[10]);
    if(multiacpl_x+chpw_w != gridWidth) painter->drawLine(DRAW_GRID_START_X+multiacpl_x+chpw_w,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpl_x+chpw_w,globalUserVariable->gridYPos[10]);
    //Johnson 190326 draw arrow line
    painter->drawText(DRAW_GRID_START_X+multiacpl_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, "<");
    painter->drawLine(DRAW_GRID_START_X+multiacpl_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2, DRAW_GRID_START_X+chpw_w+multiacpl_x, DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2);
    painter->drawText(DRAW_GRID_START_X+multiacpl_x+chpw_w-fm.width(">") , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, ">");
    //Johnson 190326 draw dbm value
    powerValue = QString::number(dPowerValue_MultiACPL,'f',2);
    painter->drawText(DRAW_GRID_START_X+multiacpl_x+chpw_w/2-fm.width(powerValue)/2,DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2 + 5,fm.width(powerValue),fm.height(),Qt::AlignLeft,powerValue);
    //********draw left MultiACP end*************

    //********draw rigth MultiACP begin***********
    painter->setPen(Qt::white);
    if(multiacpr_x != 0) painter->drawLine(DRAW_GRID_START_X+multiacpr_x,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpr_x,globalUserVariable->gridYPos[10]);
    if(multiacpr_x+chpw_w != gridWidth) painter->drawLine(DRAW_GRID_START_X+multiacpr_x+chpw_w,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpr_x+chpw_w,globalUserVariable->gridYPos[10]);
    //Johnson 190326 draw arrow line
    painter->drawText(DRAW_GRID_START_X+multiacpr_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, "<");
    painter->drawLine(DRAW_GRID_START_X+multiacpr_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2, DRAW_GRID_START_X+chpw_w+multiacpr_x, DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2);
    painter->drawText(DRAW_GRID_START_X+multiacpr_x+chpw_w-fm.width(">") , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, ">");
    //Johnson 190326 draw dbm value
    powerValue = QString::number(dPowerValue_MultiACPR,'f',2);
    painter->drawText(DRAW_GRID_START_X+multiacpr_x+chpw_w/2-fm.width(powerValue)/2,DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2 + 5,fm.width(powerValue),fm.height(),Qt::AlignLeft,powerValue);
    //*******draw rigth MultiACP end***************

    //********draw Far rigth MultiACP begin***********
    painter->setPen(Qt::white);
    if(multiacpFarr_x != 0) painter->drawLine(DRAW_GRID_START_X+multiacpFarr_x,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpFarr_x,globalUserVariable->gridYPos[10]);
    if(multiacpFarr_x+chpw_w != gridWidth) painter->drawLine(DRAW_GRID_START_X+multiacpFarr_x+chpw_w,DRAW_GRID_START_Y,DRAW_GRID_START_X+multiacpFarr_x+chpw_w,globalUserVariable->gridYPos[10]);
    //Johnson 190326 draw arrow line
    painter->drawText(DRAW_GRID_START_X+multiacpFarr_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, "<");
    painter->drawLine(DRAW_GRID_START_X+multiacpFarr_x , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2, DRAW_GRID_START_X+chpw_w+multiacpFarr_x, DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2);
    painter->drawText(DRAW_GRID_START_X+multiacpFarr_x+chpw_w-fm.width(">") , DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5 , fm.width("<"), fm.height(), Qt::AlignLeft, ">");
    //Johnson 190326 draw dbm value
    powerValue = QString::number(dPowerValue_MultiACPFR,'f',2);
    painter->drawText(DRAW_GRID_START_X+multiacpFarr_x+chpw_w/2-fm.width(powerValue)/2,DRAW_GRID_START_Y+3*SA_guiDrawSet.gridVerStep/5+fm.height()/2 + 5,fm.width(powerValue),fm.height(),Qt::AlignLeft,powerValue);
...
void sa_UIRefresh_Control::drawMeasureRect(QPainter *painter)
{
    ...
    if (globalUserVariable->currentMode != SA_SPAN_MODE)
    {
...
#else
       gridWidth = mPixmap.width() - SA_LEFTDIS - SA_RIGHTDIS;
#endif
        qreal FZ = (qreal)globalUserVariable->gChannelBandwidth;
        qreal SA_FM = (qreal)globalUserVariable->gSpanFreq / gridWidth;
        chpw_w = (qint32)(FZ/SA_FM);
        if (chpw_w < BW_MIN)
        {
       ...
       case SA_ACP_MEASURE_MODE:
            drawACPPower(painter,chpw_x,chpw_w,pixelStep,gridWidth);
       ...
       case SA_MULTI_ACP_MODE:
            drawMultiACPPower(painter,chpw_x,chpw_w,pixelStep,gridWidth);
            if(globalUserVariable->dTmpChannelBandwidth != globalUserVariable->gChannelBandwidth ||
               globalUserVariable->dTmpChannelSpacing != globalUserVariable->gChannelSpacing)
               emit(refreshMeasureSet());
            break;
       case SA_CHNLPOWER_MEASURE_MODE:
            drawChannelPower(painter,chpw_x,chpw_w,pixelStep,gridWidth);
            if(globalUserVariable->dTmpChannelBandwidth != globalUserVariable->gChannelBandwidth)
               emit(refreshMeasureSet());
            break;
        case SA_OCCUPIED_BW_MEASURE_MODE:
            drawOccupiedBWPower(painter,pixelStep);
            break;
...
void sa_UIRefresh_Control::drawSpectrumEmissionMask(QPainter *painter)
{
    ...
    qint32 offset_1 = SPEC_EM_CENT_OFFSET_1;
    qint32 offset_2 = SPEC_EM_CENT_OFFSET_2;
    qint32 offset_3 = SPEC_EM_CENT_OFFSET_3;
    qint32 offset_4 = SPEC_EM_CENT_OFFSET_4;
          ...
          else if (i == (globalUserVariable->gSweepTracePoint - 1))
          {
              ...
              if ((draw_point[currentTraceNo][i].y()  < limitUpperYAxis)  && (limitUpperYAxis != 0.) )
              {
                  ...
                  if ((draw_point[currentTraceNo][i - 1].y() > limitUpperYAxis) && (limitUpperYAxis != 0.))
                  {
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 + ((i - 1) * pixelStep), limitLowerYAxis, DRAW_GRID_START_X + 1 + position_center,  limitLowerYAxis);
              ...
              if ( (draw_point[currentTraceNo][i].y()  > limitLowerYAxis) && (limitLowerYAxis != 0.) )
              {
                  ...
                  {
                      painter->setPen(mTraceColor[currentTraceNo]);
                      painter->drawLine(DRAW_GRID_START_X + 1 + ((i - 1) * pixelStep), limitUpperYAxis, DRAW_GRID_START_X + 1 + position_center,   limitLowerYAxis);

                      globalUserLimit->gLimitTestResult[globalUserLimit->gLimitSelect] = SA_FAIL;
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 + position_center, limitLowerYAxis, DRAW_GRID_START_X + 1 + (i * pixelStep),  draw_point[currentTraceNo][i].y());
                  }
                  else
                  {
                      globalUserLimit->gLimitTestResult[globalUserLimit->gLimitSelect] = SA_FAIL;
              ...
              else
              {
                  if ((draw_point[currentTraceNo][i - 1].y() > limitLowerYAxis) && (limitLowerYAxis != 0.))
                  {
                      ...
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  ((i - 1) * pixelStep), draw_point[currentTraceNo][i - 1].y(), DRAW_GRID_START_X + 1 + position_center,  limitLowerYAxis);

                      painter->setPen(mTraceColor[currentTraceNo]);
                      painter->drawLine(DRAW_GRID_START_X + 1 + position_center, limitLowerYAxis, DRAW_GRID_START_X + 1 + (i * pixelStep),  limitUpperYAxis);
                  }
                  else
                  {
          ...
          else
          {
              ...
              if ((limitUpperYAxis != 0.) && (limitLowerYAxis != 0.))
              {
                  ...
                  if ((draw_point[currentTraceNo][i - 1].y() < limitUpperYAxis) && (draw_point[currentTraceNo][i].y() > limitLowerYAxis))
                  {
                      ...
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  ((i - 1) * pixelStep), draw_point[currentTraceNo][i - 1].y(), DRAW_GRID_START_X + 1 + (qint32) upper_pos_axis,  limitUpperYAxis);

                      painter->setPen(mTraceColor[currentTraceNo]);
                      painter->drawLine(DRAW_GRID_START_X + 1 +   (qint32) upper_pos_axis, limitUpperYAxis, DRAW_GRID_START_X + 1 + (qint32) lower_pos_axis,  limitLowerYAxis);

                      //painter->setPen(Qt::gray);  // QColor/QPenglobalUserVariable
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  (qint32) lower_pos_axis, limitLowerYAxis, DRAW_GRID_START_X + 1 + (i  * pixelStep),  draw_point[currentTraceNo][i].y());
                  }
                  else if ((draw_point[currentTraceNo][i - 1].y() > limitLowerYAxis)  && (draw_point[currentTraceNo][i].y() < limitUpperYAxis ))
                  {
                      ...
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  ((i - 1) * pixelStep), draw_point[currentTraceNo][i - 1].y(), DRAW_GRID_START_X + 1 + (qint32) lower_pos_axis,  limitLowerYAxis);

                      painter->setPen(mTraceColor[currentTraceNo]);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  (qint32) lower_pos_axis, limitLowerYAxis, DRAW_GRID_START_X + 1 +  (qint32) upper_pos_axis,  limitUpperYAxis);

                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + 1 +  (qint32) upper_pos_axis, limitUpperYAxis, DRAW_GRID_START_X + 1 + (i  * pixelStep),  draw_point[currentTraceNo][i].y());
                  ...
                  {
                      painter->setPen(mTraceColor[currentTraceNo]);
                      painter->drawLine(DRAW_GRID_START_X + 1 +   ((i - 1) * pixelStep), draw_point[currentTraceNo][i - 1].y(), DRAW_GRID_START_X + 1 +  (qint32) position_center,  limitUpperYAxis);
                  ...
                  else if ((draw_point[currentTraceNo][i - 1].y() > limitUpperYAxis) && (draw_point[currentTraceNo][i].y() >= limitLowerYAxis ))
                  {
                      ...
                      //painter->setPen(Qt::gray);  // QColor/QPen
                      painter->setPen(mTrOutLimitColor);
                      painter->drawLine(DRAW_GRID_START_X + position_center, limitLowerYAxis, DRAW_GRID_START_X + (i  * pixelStep),   draw_point[currentTraceNo][i].y());
                  }
                  else
                  {
                      painter->setPen(mTraceColor[currentTraceNo]);
...
void sa_UIRefresh_Control::drawMarkerCurrentSelectInfo(QPainter *painter)
{
    ...
    if (globalUserVariable->gMarkerView[i] == SA_ON)
    {
        ...
        if (globalUserVariable->gMarkerType[i] == REF)
        {
            ...
            if(!globalUserVariable->gMarkerFreqCount[globalUserVariable->gMarkerSelect]) strLabel = QString("M%1").arg(i + 1);
            ...
            //Johnson 190621
            if(globalUserVariable->gMarkerFreqCount[globalUserVariable->gMarkerSelect])
            {
...
/**
 * @brief sa_UIRefresh_Control::Delta_dbmToVoltCalculatorAndDisplayFunc
 * @param label
 * @param _level_Delta
 * @param _level_Ref
 * @author Johnson
 */
void sa_UIRefresh_Control::Delta_dbmToVoltCalculatorAndDisplayFunc(char *label, qreal _level_Delta, qreal _level_Ref)
{
    qreal voltGab = 0.;
    qreal calculator_Delta = 0. , calculator_Ref = 0.;
    bool minus = false;

    calculator_Delta = pow(10, _level_Delta / 10.);
    calculator_Ref = pow(10, _level_Ref / 10.);
    voltGab = 1000000. * pow(50 * calculator_Delta * 0.001, 0.5) - 1000000. * pow(50 * calculator_Ref * 0.001, 0.5);//uv

    if(voltGab >= 0.) minus = false;
    else minus = true;
...
/**
 * @brief sa_UIRefresh_Control::Delta_dbmToWattsCalculatorAndDisplayFunc
 * @param label
 * @param _level_Delta
 * @param _level_Ref
 */
void sa_UIRefresh_Control::Delta_dbmToWattsCalculatorAndDisplayFunc(char *label, qreal _level_Delta, qreal _level_Ref)
{
    qreal wattsGab = 0.;
    bool minus = false;
    wattsGab = pow(10, _level_Delta / 10.) - pow(10, _level_Ref / 10.);
    if(wattsGab >= 0.) minus = false;
    else minus = true;
    ...
    if(!minus)
    {
        //Johnson 0919 Change
        if (wattsGab >= 1000) {						// W
            sprintf(label, "%.2f", (qreal)wattsGab / 1000.0);
          }  else if (wattsGab >= 1) {					// mW
            sprintf(label, "%.2f", (qreal)wattsGab);
          }  else if (wattsGab >= 0.001) {				// uW
            sprintf(label, "%.2f", (qreal)wattsGab * 1000.0);
          }  else if (wattsGab >= 0.000001) {			// nW
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000.0);
          } else if (wattsGab >= 0.000000001) {			// pW
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000000.0);
          } else if (wattsGab >= 0.000000000001) {		// fW
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000000000.0);
          } else if (wattsGab >= 0.000000000000001) {		// aW
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000000000000.0);
            ...
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000000000000000000.0);
          } else {
            sprintf(label, "%.2f", (qreal)wattsGab * 1000000000000000000.0);
          }
    }
...
void sa_UIRefresh_Control::mouseMoveReflect(qint32 x,qint32 y,bool bSmall)
{
    if (marker_mouse_move_pos == SA_MOUSE_ACTION)
    {
        ...
        if(bSmall)
        {
            calculatorXPos = (x - DRAW_GRID_START_X) / smallPixmapPixelStep;
        }
        else
        {
            calculatorXPos = (x - DRAW_GRID_START_X) / largePixmapPixelStep;
        }
        */
        calculatorXPos = (x - DRAW_GRID_START_X) / mPixmapPixelStep;
...
void sa_UIRefresh_Control::judgeSpecEMTestResult()
{
    ...
    qint32 startIndex[2 * SA_SPECEM_FREQ_CNT],stopIndex[2 * SA_SPECEM_FREQ_CNT];
    ...
    for(qint32 i = 0;i < SA_SPECEM_FREQ_CNT;i++)
    {
        ...
        startIndex[SA_SPECEM_FREQ_CNT+i] = upperStart;
        stopIndex[SA_SPECEM_FREQ_CNT+i]  = upperStop;
    }
     for(qint32 i = 0;i<2*SA_SPECEM_FREQ_CNT;i++)
    {
        globalUserVariable->gSpecEMTestResult[i] = SA_PASS;
        if(i<SA_SPECEM_FREQ_CNT)
        {
            if(globalUserVariable->gSpecEMViewStatus[i] == SA_OFF) continue;
        }else
        {
            if(globalUserVariable->gSpecEMViewStatus[i-5] == SA_OFF) continue;
        }
...
//refresh mkr
void sa_UIRefresh_Control::sendRemoteMkrCmd()
{
    //if(globalUserVariable->gRunMode != SA_REMOTE) return;
    qreal value;
    if (globalUserVariable->gMarkerType[globalUserVariable->gMarkerSelect] == REF)
    {
        if(globalUserVariable->gZeroSpanAction == SA_OFF)
          value = globalUserVariable->gMarkerRefFreq[globalUserVariable->gMarkerSelect];
        else
          value =  globalUserVariable->gMarkerRefTime[globalUserVariable->gMarkerSelect];
    }
    else
    {
        if(globalUserVariable->gZeroSpanAction == SA_OFF)
          value = globalUserVariable->gMarkerDeltaFreq[globalUserVariable->gMarkerSelect];
        else
          value = globalUserVariable->gMarkerDeltaTime[globalUserVariable->gMarkerSelect];
    }

    QString strCmd;
    QString iVal = QString::number(value);
    strCmd = QString(":MARKer:X ");
    strCmd += iVal;
    emit refreshRemote(strCmd);
}
