...
//Johnson 0906
void user_limit::limitXAxisCalculatorFunc(quint64 Freq,quint32 index,quint32 uLimitSelect)
{
    //Freq = Freq * SA_MHZ;

    // Limit Line Freq Position Find
    for (qint32 k = 0; k < (globalUserVariable->gSweepTracePoint + 1); k++) {
        quint64 tempFreq =  globalUserVariable->gStepFreq * k;
        quint64 vigeoFreq = globalUserVariable->gStartFreq + tempFreq;
