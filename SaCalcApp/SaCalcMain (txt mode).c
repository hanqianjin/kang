...
extern void SetIfAtten(DiSession devHandle);
int main(int argc, char *argv[])
{
	...
	/*
	printf("iBack is %d\n",iBack);
        FILE* fp1 = fopen("freqlistb.ini","wb+");
	fwrite("No.       freqHz          detadB\n",sizeof("No.       freqHz          detadB\n"),1,fp1);	
        for(int i = 0;i < 10;i++)
	{
		printf("%lf\n",*(bitFreqList.param+i));
		fwrite(bitFreqList.param+i,sizeof(double),1,fp1);
	}      
        //fclose(fp1);
	//fp1 = fopen("freqlistb.ini","rb");
	rewind(fp1);
	double val = 0.0;
	char t[256];
	iBack = fread(t,sizeof("No.       freqHz          detadB\n"),1,fp1);
 	printf("%s",t);
        for(int k = 0;k < 10;k++)
	{
		iBack = fread(&val,sizeof(double),1,fp1);
                printf("num is %d,index is %d,the freq val is %lf\n",iBack,k,val);
	}
 	fclose(fp1);
	return 0;
        */
