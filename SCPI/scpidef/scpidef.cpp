...
scpi_result_t COMM_ModeChange(scpi_t * context) {
    ...
    if (param){
        ...
        case Port_internet:
            if (!LocalCommunicate::instance()->changeMode(Mode_remote_internet)){
                return SCPI_RES_ERR;
            } else {
            ...
            break;
        case Port_usb:
            if (!LocalCommunicate::instance()->changeMode(Mode_remote_usb))
                return SCPI_RES_ERR;
            else
            ...
            break;
        case Port_gpib:
            if (!LocalCommunicate::instance()->changeMode(Mode_remote_gpib))
                return SCPI_RES_ERR;
            else
            ...
            break;
        case Port_uart:
            if (!LocalCommunicate::instance()->changeMode(Mode_remote_uart))
                return SCPI_RES_ERR;
            else
...
static const scpi_command_t scpi_commands[] = {
    ...
    /*SCPI_COMMANDS_DISPLAY_BAR
    SCPI_COMMANDS_DISPLAY_FFT
    SCPI_COMMANDS_DISPLAY_FLICKER
    SCPI_COMMANDS_DISPLAY_IECHARMONIC
    SCPI_COMMANDS_DISPLAY_MOTOR
    SCPI_COMMANDS_DISPLAY_NUMERIC
    SCPI_COMMANDS_DISPLAY_TREND
    SCPI_COMMANDS_DISPLAY_UTILITY
    SCPI_COMMANDS_DISPLAY_VECTOR
    SCPI_COMMANDS_DISPLAY_WAVE
    SCPI_COMMANDS_DISPLAY_STORE
    SCPI_COMMANDS_FILE
    SCPI_COMMANDS_INPUT
    SCPI_COMMANDS_INTEGRATE
    SCPI_COMMANDS_MEASURE
    SCPI_COMMANDS_NUMERIC
    SCPI_COMMANDS_RANGE
    SCPI_COMMANDS_SYSTEM
    SCPI_COMMANDS_USER
    SCPI_COMMANDS_UTILITY*/
    SCPI_CMD_LIST_END
};
...
scpi_t scpi_context = {
    ...
    /* user_context */ NULL,
    /* idn */ {"INNO", "PowerAnalyzer", NULL, "01-02"},
};
