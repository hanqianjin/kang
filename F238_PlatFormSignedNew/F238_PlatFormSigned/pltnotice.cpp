  ...
  ui(new Ui::pltnotice)
{
  ...
  //upgrade condition is OK
  else if(PTY3 == pltnot.istyle)
    {
         ui->Warning->setText("Please keep power above 30%");
    }
  //upgrade conditon is error,PTY4 means dir not,PTY5 means packet not
  else if(PTY4 == pltnot.istyle || PTY5 == pltnot.istyle)
    {
      ui->pushButtonok->hide();
      ui->pushButtoncancel->move(440,118);

      if(PTY4 == pltnot.istyle)
        {
          ui->Warning->setText("The upgrade storage media not found!");
        }
      else
        {
          ui->Warning->setText("The upgrade packet not found!");
        }
    }
}
...
#ifdef plt_bit_238_smart
bool pltnotice::event(QEvent *event)
{
  if(PTY2 == pltnot.istyle)
    {
        if(pbkbhandle)
...
void pltnotice::on_lineEdit_clicked()
{
  if(NULL == pbkbhandle)
    {
      pbkbhandle = new bitboard(this);
      pbkbhandle->move(this->pos().x(),this->pos().y()+this->height());
    }
  else if(pbkbhandle->isHidden())
    {
      pbkbhandle->show();
    }
}
