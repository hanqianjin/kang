...
** Latest modified Date:  2019-11-13
** Modified by:steve
...
#define FILE_REFRESH_SIZE   (2 * 1024 * 1024) //max log file size
...
{
    ui->setupUi(this);
    QApplication::setOverrideCursor(Qt::ArrowCursor/*BlankCursor*/);//Johnson 20200317
    // delete log before
    checkLogFileSize();
    initNoticeCode(PTY1);
    ...
    {
        qCritical("7");
        setNoticePara(PTY1,PTY2,7,PLT_EXCE_SOURCE);
    ...
    pltlastallstates.ilastallmesa[1] = 0;
    ...
    if(LoadAllTimer() < 0)
    {
        //dealing expec
        qFatal("4");
        setNoticePara(PTY1,PTY2,4,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    ConnectAllSigSlot();
    BatPicPltdis(pltlastallstates.dismod);
    SetRmdTemper();
}
...
{    
    ...
    {
        shmengine::getsingleshm()->pshm_sysconf->AnaMode = PUZERO;
        PltToMainExit();
        shmengine::getsingleshm()->pshm_calnotic->notic = PUZERO;
    }
    usleep(1000000);
    DeleteW();
#ifdef _Linux_F238_Log
...
void BitPlatForm::IniCtrlVec()
{
    ...
    noticexcepicvec.append(EXCENOTIC);
    noticexcepicvec.append(EXCENOTICNIGHT);

    noticexcetoolpicvec.append(TOOLNOTIC);
    noticexcetoolpicvec.append(TOOLNOTICNIGHT);

    plusnorvec.append(PLUSPICNORMAL);
    plusclickvec.append(PLUSPICCLICK);
    return;
}
 ...
 * @return
 * @note
 * @since {2019-01-09;c12-19}
*/
void BitPlatForm::Uphomeapp()
{
    homeprocess.start("./HomeApp",QStringList("./HomeApp"));
    if(!homeprocess.waitForStarted(CTRLUPOVERTIME))
    {
        qCritical("15");
        setNoticePara(PTY1,PTY2,15,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    upgradeprocess.start("./BitGradeApp",QStringList("./BitGradeApp"));
    if(!upgradeprocess.waitForStarted(CTRLUPOVERTIME))
    {
        qCritical("21");
        setNoticePara(PTY1,PTY2,21,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    return;
}
...
void BitPlatForm::PltIniSa()
{

}

void BitPlatForm::PltIniRtsa()
{

}

void BitPlatForm::PltIniIa()
{

}

void BitPlatForm::PltIniPm()
{

}

void BitPlatForm::PltIniFdd()
{

}

void BitPlatForm::PltIniTdd()
{

}

void BitPlatForm::PltIniFgnr()
{
...
void BitPlatForm::InnerTimeCtrl(qint32 tid)
{
    if(sysmsgtimerhandle == tid)
    {
        ...
        //        }
        //deal with excenotice
        if(PTY0 != shmengine::getsingleshm()->pshm_pltnotic->noticode)
        {
            emit Pltexcenotsig(shmengine::getsingleshm()->pshm_pltnotic->noticode);
            shmengine::getsingleshm()->pshm_pltnotic->noticode = PTY0;
        }
 ...
 * @since {2019-11-04}
*/
void BitPlatForm::QueryBatAndGps()
{
    qdeta = PTY0;
    if(BURN_STATE)
    {
        ...
        if(pltlastallstates.gpswitch)
        {
            ...
            //reflesh statebar states
            pSetWid->pnetsetwid->ReflesGpsLabelStatus(gpsflag);
            QString strLatitude = QString(QLatin1String(shmengine::getsingleshm()->pshm_sysinfo->GPSLatitude));
            strLatitude = ChangeGPSDisFormat(strLatitude);
            QString strLongtitude = QString(QLatin1String(shmengine::getsingleshm()->pshm_sysinfo->GPSLongitude));
            strLongtitude = ChangeGPSDisFormat(strLongtitude);
            ui->gpslabel->setText(strLatitude + " " + strLongtitude);
//            ui->gpslabel->setText(QString(QLatin1String(shmengine::getsingleshm()->pshm_sysinfo->GPSLatitude)) + " " +
//                                  QString(QLatin1String(shmengine::getsingleshm()->pshm_sysinfo->GPSLongitude)));
        }
        else
 ...
 * @since {2019-10-30}
*/
void BitPlatForm::SetRmdTemper()
{
    QStringList templist;
    templist.append(QString::number(shmengine::getsingleshm()->pshm_sysinfo->BoardTemp) + " deg");
    templist.append(QString::number(shmengine::getsingleshm()->pshm_sysinfo->RfTemp) + " deg");
    templist.append(QString::number(shmengine::getsingleshm()->pshm_sysinfo->FpgaTemp) + " deg");
    templist.append(QString::number(shmengine::getsingleshm()->pshm_sysinfo->BatteryTemp) + " deg");
    pSetWid->psyswid->SetTemperSysinfo(templist);
}
...
PlusPicPltdis(pltlastallstates.dismod,pltlastallstates.nowshowmodsum - 1);
return;
}
...
void BitPlatForm::ConnectAllSigSlot()
{
    ...
    QObject::connect(this, SIGNAL(Pltexcenotsig(quint32)), this, SLOT(DoexcenotAciton(quint32)));
    return;
}
...
{   
    ...
    pcirlmsgqueue = new pltcirlist(PLTMAXCIRLEN);
    perrwid = new noticmsg(this);
    ...
    // QObject::connect(pLtefddAna,SIGNAL(DemodAutoMeasSound(qint32,qint32)),this,SLOT(PltPlayWav(qint32,qint32)));
    ...
    //  QObject::connect(pLtetddAna,SIGNAL(DemodAutoMeasSound(qint32,qint32)),this,SLOT(PltPlayWav(qint32,qint32)));
    ...
    QObject::connect(pFgnrAna,SIGNAL(DemodAutoMeasSound(qint32,qint32)),this,SLOT(PltPlayWav(qint32,qint32)));
 ...
 * @since {2019-10-21}
*/
void BitPlatForm::ExcePicdis(qint32 imode, qint32 icome)
{
    if(imode < PTY0 || imode > PTY2)
        return;
    if(icome < PTY0 || icome > PTY1)
        return;
    if(PTY1 == imode)
        imode = PTY0;
    if(PTY2 == imode)
        imode = PTY1;
    ui->label_10->setStyleSheet(QString("QLabel {border-image: url(%1);}").arg(QString::fromUtf8(noticexcetoolpicvec.at(imode)[icome])));
}
 ...
 * @since {2019-11-18}
*/
void BitPlatForm::PlusPicPltdis(qint32 imode, qint32 inum)
{
    if(PTY0 == imode)
    {
        plusnorvec.replace(PTY0,PLUSPICNORMAL);
        plusclickvec.replace(PTY0,PLUSPICCLICK);
    }
    else if(PTY1 == imode)
    {
        plusnorvec.replace(PTY0,PLUSPICNORMAL);
    ...
    {
        plusnorvec.replace(PTY0,PLUSPICNORMALNIGHT);
        plusclickvec.replace(PTY0,PLUSPICCLICKNIGHT);
    }
    //just set style
    QString mstr = QString("QPushButton{background-image: url(%1);border-style: flat;}\
                           QPushButton:pressed{background-image: url(%2);border-style: flat;}")
                           .arg(QString::fromUtf8(plusnorvec.at(PTY0)[inum]),QString::fromUtf8(plusclickvec.at(PTY0)[inum]));
            ui->plusbtn->setStyleSheet(mstr);
}
...
{  
    ...
    qint32 itime = 0;
    BURN_STATE = POFF;
    while(timerstate || shmengine::getsingleshm()->pshm_sysaction->top_flag_sys)
        //        || shmengine::getsingleshm()->pshm_sysaction->top_flag_conf)
    {
        itime++;
        if(itime > (BURNTIMEOUT >> PTY2))
        {
            qCritical()<<"Free sync:"<<shmengine::getsingleshm()->pshm_sysaction->top_flag_sys;
            shmengine::getsingleshm()->pshm_sysaction->top_flag_sys = PTY0;
            break;
        }
        usleep(SYSINTER);
    };
...
bool BitPlatForm::StartCtrlApp(quint32 modetype)
{
    ...
    if(mstr != "bit")
    {
        ...
        if(!processvec.at(modetype)->waitForStarted(CTRLUPOVERTIME))
        {
            qCritical("14");
            setNoticePara(PTY1,PTY2,14,PLT_EXCE_SOURCE);
            shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
        }
    }
    return true;
...
{    
    ...
    {
        qCritical() << "start cut sec fpga picture failed";
        return;
    }
    ...
    setNoticePara(PTY0,PTY0,19,PLT_EXCE_SOURCE);
    shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
...
{
    if(Qt::Key_PowerOff/*Key_F4*/ == event->key())
    {
 ...
 * @since {2019-10-14}
*/
void BitPlatForm::DoexcenotAciton(quint32 icode)
{
    excenoti::anaexcenoticode(icode);
    //err or notic or other type logo
    short np = excenoti::getexecnotihandle()->codemsg.npflag;
    //err type
    short etype = excenoti::getexecnotihandle()->codemsg.ntype;
    //source
    short asource = excenoti::getexecnotihandle()->codemsg.source;
    //logic
    short aclogic = excenoti::getexecnotihandle()->codemsg.logicmode;
    //index
    short mindex = excenoti::getexecnotihandle()->codemsg.index;
    if(perrwid)
    {
        QString msgstr = QString::number(asource);
        //business done below
        if(!np)
        {
            etype = 0;
        ...
        msgstr += QString::number(etype);
        const char* tempp = GetShowExceNotiMsg(asource,mindex);
        qint32 inum = 0;
        if(tempp)
        {
            pcirlmsgqueue->InsertNodeValue(np,asource,mindex);
            inum = pcirlmsgqueue->Count();
        }
        else
        {
            tempp = excenoti::unknowdic[0];
            pcirlmsgqueue->InsertNodeValue(np,asource,-1);
            inum = pcirlmsgqueue->Count();
        }
        //change pic
        if(inum > 9)
        ...
        {
            ui->label_11->setText(QString::number(inum));
        }
        //
        ExcePicdis(pltlastallstates.dismod,PTY1);
        perrwid->InsertMsg(np,msgstr,tempp);
    }
    return;
}

const char* BitPlatForm::GetShowExceNotiMsg(short imode, short index)
{
    if(index > excenoti::execsumlist[imode] || index < 0)
    {
        return NULL;
    }
    return excenoti::alldictionary[imode][index];
}
...
void BitPlatForm::SetRemoteUi()
{
    ...
    if(perrwid)
    {
        ...
        {
            perrwid->hide();
            perrwid->setWindowModality(Qt::NonModal);
        }
    }
 ...
 * @since {2019-11-26}
*/
void BitPlatForm::StartUpGrade(qint32 iflag)
{
    if(!pSetWid->isHidden())
        pSetWid->hide();
    gradeflag = iflag;
        ...
        if(ist < 0)
        {
            ...
            if(pltlastallstates.nowshowmodsum <= UIMODMAXNUM)
            {
                ...
                else
                {
                    ...
                    PlusPicPltdis(pltlastallstates.dismod,modbtnindex);
                }
            }
        }
        else
...
qint32 BitPlatForm::PltSaveDevelopMsg()
{
    ...
    if(!pltdevmsg.open(QIODevice::WriteOnly))
    {
        qCritical("20");
        setNoticePara(PTY1,PTY2,20,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
        return -1;
    }
...
qint32 BitPlatForm::PltLoadDevelopMsg()
{
    ...
    if(!pltdevfile.open(QIODevice::ReadOnly))
    {
        qCritical("8");
        setNoticePara(PTY1,PTY2,8,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
        return -1;
    }
    ...
    {
        qCritical("9");
        setNoticePara(PTY1,PTY1,9,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    pltdevfile.close();
    return iwnum;
...
//     modeintervec.at(pltlastallstates.orimode)->SetDisplayMode(imod);  //zcx change
    return 0;
}
...
qint32 BitPlatForm::PltDismodForstart(qint32 imod)
{
    ...
    TopTabPicdis(imod);
    PlusPicPltdis(imod,pltlastallstates.nowshowmodsum - 1);
    CtrlModePltdis(pltlastallstates.dismod,loreflag);
    GpsPltdisMod(imod,pltlastallstates.gpswitch);
    BatPicPltdis(imod);
    ExcePicdis(imod,!!pcirlmsgqueue->Count());
    perrwid->SetPltExceDisM(imod);
    perrwid->ChangeNodeDismode(imod,*pcirlmsgqueue);
    return 0;
}

void BitPlatForm::SetExceNoticClear()
{
    perrwid->ClearAllExceNotic();
}

//Johnson 20200528
qreal BitPlatForm::getRfTemp()
{
    return shmengine::getsingleshm()->pshm_sysinfo->RfTemp;
}
qreal BitPlatForm::getBoardTemp()
{
    return shmengine::getsingleshm()->pshm_sysinfo->BoardTemp;
}
qreal BitPlatForm::getFpgaTemp()
{
    return shmengine::getsingleshm()->pshm_sysinfo->FpgaTemp;
}
float BitPlatForm::getBatteryTemp()
{
    return shmengine::getsingleshm()->pshm_sysinfo->BatteryTemp;
}

//Johnson 20200615
void BitPlatForm::refreshPltConfig(const PLTSTATEPARAM rcvCofig)
{
    PLTSTATEPARAM mpltlastallstates;
    memcpy(&mpltlastallstates,&rcvCofig,sizeof(PLTSTATEPARAM));
    ...
    //SetLocalUi(Qt::Key_Escape);
    //ui->gpslabel->setText(tr("-- -- -- --"));
    SwitchAnaMode(mpltlastallstates.orimode,mpltlastallstates.ilastallmesa[mpltlastallstates.orimode]);
    return;
}

void BitPlatForm::GetPltParam(PLTSTATEPARAM *dest)
{
    memcpy(dest,&pltlastallstates,sizeof(PLTSTATEPARAM));
}
...
{
    QFile pltprefile("plt.pres");
    if(!pltprefile.open(QIODevice::ReadOnly))
    {
        qCritical("10");
        setNoticePara(PTY1,PTY2,10,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    ...
    {
        qCritical("11");
        setNoticePara(PTY1,PTY1,11,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    pltprefile.close();
    return iwnum;
...
pltcirlist *BitPlatForm::GetCirHandle()
{
    return pcirlmsgqueue;
}

void BitPlatForm::ClearExceUi()
{
    ui->label_11->setText(" ");
    ExcePicdis(pltlastallstates.dismod,PTY0);
}
...
qint32 BitPlatForm::PltLoadStates(qint32 itype)
{
    ...
    {
        QFile pltdefaultfile("plt_default.cond");
        bl = pltdefaultfile.open(QIODevice::ReadOnly);
        iwnum = pltdefaultfile.read((char*)&pltlastallstates,sizeof(PLTSTATEPARAM));
        pltdefaultfile.close();
    }
    else if(PTY1 == itype)
    {
        QFile pltlastfile("plt_last.cond");
        bl = pltlastfile.open(QIODevice::ReadOnly);
        iwnum = pltlastfile.read((char*)&pltlastallstates,sizeof(PLTSTATEPARAM));
        pltlastfile.close();
    }
    else if(PTY2 == itype)
    {
        QFile pltuserfile("plt_user.cond");
        bl = pltuserfile.open(QIODevice::ReadOnly);
        iwnum = pltuserfile.read((char*)&pltlastallstates,sizeof(PLTSTATEPARAM));
        pltuserfile.close();
    }
    if(!bl)
    {
        qCritical("12");
        setNoticePara(PTY1,PTY2,12,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    if(iwnum < 0)
    {
        qCritical("13");
        setNoticePara(PTY1,PTY1,13,PLT_EXCE_SOURCE);
        shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
    }
    return iwnum;
}
...
void BitPlatForm::on_label_10_clicked()
{
    if(loreflag)
    {
        return;
    }
    perrwid->setWindowModality(Qt::ApplicationModal);
    perrwid->show();
}

void BitPlatForm::on_label_11_clicked()
{
    if(loreflag)
    {
        return;
 ...
 * @since {2019-10-31}
*/
void BitPlatForm::on_label_4_clicked()
{
    if(loreflag)
    {
        SetLocalUi(Qt::Key_Escape);
...
QString BitPlatForm::ChangeGPSDisFormat(QString str)
{
    ...
    return degreePart + u8"°" + minutePart + "'" + secondPart + "''" + tailPart;
}

void BitPlatForm::checkLogFileSize()
{
    QStringList list;
    list << "gui.log" << "sactrl.log" << "rtsactrl.log" << "inter.log" << "pm.log" << "ltefdd.log"
         << "ltetdd.log" << "fgnr.log" << "sysctrl.log" << "sysctrlerr.log";
    for(QString str : list)
    {
        ...
        if(file.exists())
        {
            ...
            {
                if(file.size() >= FILE_REFRESH_SIZE)
                {
                    file.close();
