...
qint32 shmengine::inishmengine()
{
  void* temptr = 0;
  //plt and all modes
  shm_pltnoti_id = shmget((key_t)PLT_ALL_SHM_KEY,sizeof(ALL_NOTIC_CODE),IPC_CREAT | 0666);
  if  (-1 == shm_pltnoti_id)
    {
      qCritical(strerror(errno));
      //      setNoticePara(PTY1,PTY2,0,PLT_EXCE_SOURCE);
      //      shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
      return errno;
    }
  else
    {
      temptr= shmat(shm_pltnoti_id, (void*)0, 0);
      if (temptr == (void*)-1)
        {
          qCritical(strerror(errno));
          //          setNoticePara(PTY1,PTY2,1,PLT_EXCE_SOURCE);
          //          shmengine::getsingleshm()->pshm_pltnotic->noticode = GetNoticeCode();
          return errno;
        }
      pshm_pltnotic = static_cast<PALL_NOTIC_CODE>(temptr);
    }
  //platform and sys shm init
  shm_sysconf_id = shmget((key_t)SYS_CONF_KEY,sizeof(SHM_SYS_CONF),IPC_CREAT | 0666);
  if  (-1 == shm_sysconf_id)
    {
      qCritical(strerror(errno));
      setNoticePara(PTY1,PTY2,0,PLT_EXCE_SOURCE);
...
/**
 * @brief to ctrl shm for sys do action for once mode
 * @param[in]
 * @param[in]
 * @return
 * @note
 ...
 * @since {2019-02-12}
*/
qint32 shmengine::PassTrigToSys(quint32 itopflag, quint32 itemflag)
{   
  shmengine::getsingleshm()->pshm_sysaction->system_flag_conf = itemflag;
  shmengine::getsingleshm()->pshm_sysaction->top_flag_conf = itopflag;
  return 0;
}
 ...
 * @since {2019-03-26}
*/
qint32 shmengine::PassTrigToSysExt(quint32 itopflag, quint32 itemflag)
{
  shmengine::getsingleshm()->pshm_sysaction->sys_flag_info = itemflag;
  shmengine::getsingleshm()->pshm_sysaction->top_flag_sys = itopflag;
  return 0;
}
