...
#include <QListView>
RtSa_Dialog_AmptSetup::RtSa_Dialog_AmptSetup(RtSa_Interface *interface, RtSa_Dialog_Digitinput *input, QWidget *parent) :
    QWidget(parent),
...
//    connect(mUnitGroup,SIGNAL(buttonClicked(qint32)),this,SLOT(unitSelect(qint32)));
    ...
    ui->pbPreAmptOn->setChecked(mControl->IsPreAmpOn());
...
//    mUnitGroup->button(mControl->getAmptUint())->setChecked(true);
//    ui->pbAutoAttenOn->setChecked(mControl->IsAttenAuto());
//    ui->pb_AttenType->setCurrentIndex(mControl->IsAttenAuto());
    ui->pb_unitSelect->setCurrentIndex(mControl->getAmptUint());
    connect(ui->pb_unitSelect,SIGNAL(currentIndexChanged(qint32)),this,SLOT(unitSelect(qint32)));
    bool ispreampOn = mControl->IsPreAmpOn();
    if(ispreampOn)
    {
...
{
    bool isAuto = mControl->IsAttenAuto();
    if(isAuto)
    {
...
void RtSa_Dialog_AmptSetup::on_timeOut()
{
    ...
    if(!mUpDownFlag)
    {
        ...
        {
        case RTSA_INPUT_REF_LEVEL:
            mControl->setRefLevel(mControl->getRefLevel() + 1);
            initDisplay();
            break;
        case RTSA_INPUT_ATTEN:
            mControl->setAttenVal(mControl->getAttenVal() + 5);
            initDisplay();
            break;
        case RTSA_INPUT_SCALEDIV:
            mControl->setScaleDiv(mControl->getScaleDiv() + 1);
            initDisplay();
            break;
        case RTSA_INPUT_REF_OFFSET:
            mControl->setRefOffset(mControl->getRefOffset() + 1);
            initDisplay();
            break;
        default:
            break;
