...
RtSa_Dialog_TriggerSetup::RtSa_Dialog_TriggerSetup(RtSa_Interface *interface, RtSa_Dialog_Digitinput *input, QWidget *parent) :
    QWidget(parent),
    mControl(interface),
...
//    connect(ui->edit_External_level,SIGNAL(mFocus(RtSa_LineEdit*)),this,SLOT(on_getFocus(RtSa_LineEdit*)));
    ...
    //Albert fmt not use holdoff time
    ui->edit_FMT_HoldOff->hide();
    ui->label_26->hide();
...
{
    if(index < RTSA_INPUT_TRIGGER_DELAY_VALUE || index > RTSA_INPUT_TRIGGER_HOLDTIME)
        return;
    qint32 mEditIndex,mode;
    mode = mControl->getTrigger();
...
void RtSa_Dialog_TriggerSetup::refreshSubmenuDisplay()
{
    ...
    qint32 status;
    QString strInfo;
    qint32 mode = mControl->getTrigger();
...
//        break;
    case RTSA_TRIGGER_VIDEO:
        value = mControl->getTriggerLevel(RTSA_TRIGGER_VIDEO);
        strInfo = QString::number(value,'f',2);
        strInfo += QString(" dBm");
        ui->edit_Video_level->setText(strInfo);

        status = mControl->getTriggerSlope(RTSA_TRIGGER_VIDEO);
        if(status)
        {
        ...
        status = mControl->getTriggerDelayStatus(RTSA_TRIGGER_VIDEO);
        ui->pb_Video_DelayOn->setChecked(status);
        if(status)
            ui->pb_Video_DelayOn->setText("On");
        else
            ui->pb_Video_DelayOn->setText("Off");
        value = mControl->getTriggerDelay(RTSA_TRIGGER_VIDEO);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_Video_DelayValue->setText(strInfo);

        value = mControl->getriggerHoldOffTime(RTSA_TRIGGER_VIDEO);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_Video_HoldOff->setText(strInfo);
        break;
    case RTSA_TRIGGER_EXTERNAL:
        value = mControl->getTriggerLevel(RTSA_TRIGGER_EXTERNAL);
        strInfo = QString::number(value,'f',2);
        strInfo += QString(" dBm");
//        ui->edit_External_level->setText(strInfo);

        status = mControl->getTriggerSlope(RTSA_TRIGGER_EXTERNAL);
        if(status)
        {
        ...
        status = mControl->getTriggerDelayStatus(RTSA_TRIGGER_EXTERNAL);
        ...
        value = mControl->getTriggerDelay(RTSA_TRIGGER_EXTERNAL);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_External_DelayValue->setText(strInfo);
        value = mControl->getriggerHoldOffTime(RTSA_TRIGGER_EXTERNAL);
        strInfo = mControl->getAdjustTimeStr(value);
    ...
    case RTSA_TRIGGER_PERIODIC_TIMER:
        ...
        status = mControl->getTriggerDelayStatus(RTSA_TRIGGER_PERIODIC_TIMER);
        ...
        value = mControl->getTriggerDelay(RTSA_TRIGGER_PERIODIC_TIMER);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_PeriTime_DelayValue->setText(strInfo);
        break;
    case RTSA_TRIGGER_RF_BURST:
        value = mControl->getTriggerLevel(RTSA_TRIGGER_RF_BURST);
        strInfo = QString::number(value,'f',2);
        strInfo += QString(" dBm");
        ui->edit_RFBurst_level->setText(strInfo);

        status = mControl->getTriggerSlope(RTSA_TRIGGER_RF_BURST);
        if(status)
        {
        ...
        status = mControl->getTriggerDelayStatus(RTSA_TRIGGER_RF_BURST);
        ui->pb_RFBurst_DelayOn->setChecked(status);
        if(status)
            ui->pb_RFBurst_DelayOn->setText("On");
        else
            ui->pb_RFBurst_DelayOn->setText("Off");
        value = mControl->getTriggerDelay(RTSA_TRIGGER_RF_BURST);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_RFBurst_DelayValue->setText(strInfo);
        value = mControl->getriggerHoldOffTime(RTSA_TRIGGER_RF_BURST);
        strInfo = mControl->getAdjustTimeStr(value);
        ui->edit_RFBurst_HoldOff->setText(strInfo);
        break;
    case RTSA_TRIGGER_FMTMASK:
        value = mControl->getriggerHoldOffTime(RTSA_TRIGGER_FMTMASK);
        strInfo = mControl->getAdjustTimeStr(value);
...
{
    qint32 mode = mControl->getTrigger();
    qint32 critertion = mControl->getFmtTriggerCriterion();
...
//    }
    ...
    if(mControl->getFmtType() == BIT_FMT_UPPER)
    {
...
{
    mControl->setTriggerDelayStatus(RTSA_TRIGGER_EXTERNAL,checked);
    initDisplay();
    return;
}
...
{
    mControl->setTriggerDelayStatus(RTSA_TRIGGER_PERIODIC_TIMER,checked);
    initDisplay();
    return;
}
...
{
    mControl->setTriggerDelayStatus(RTSA_TRIGGER_RF_BURST,checked);
    initDisplay();
    return;
}
...
{
     mControl->setTriggerDelayStatus(RTSA_TRIGGER_VIDEO,checked);
    initDisplay();
    return;
}
...
void RtSa_Dialog_TriggerSetup::on_pb_RFBurst_Pos_clicked(bool checked)
{
    ...
    mControl->setTriggerSlope(triggerMode,checked);
    initDisplay();
    return;
}
...
void RtSa_Dialog_TriggerSetup::on_pb_FMTNew_clicked()
{
    ...
    mFmtTable = new RtSa_Dialog_FmtTable(mControl,mInputDialog,this);
    connect(mFmtTable,SIGNAL(destroyed()),this,SLOT(on_limitDestroyed()));
//    mFmtTable->
//    connect(mFmtTable,SIGNAL(destroyed()),this,SLOT(on_limitDestroyed()));
//    if(mFmtTable->isHidden())
//    {
//        mFmtTable->setGeometry(mapToGlobal(QPoint(0,0)).x()+this->width()-RTSA_DIGITAL_DIALOG_WIDTH,\
//                                  mapToGlobal(QPoint(0,0)).y(),RTSA_DIGITAL_DIALOG_WIDTH,RTSA_DIGITAL_DIALOG_HEIGHT);
//    }
    mFmtTable->show();
    mFmtTable->setFocus();
...
{
    mControl->setFmtType(BIT_FMT_UPPER);
    return;
}
...
{
    mControl->setFmtType(BIT_FMT_LOWER);
    return;
}
/**
* @brief
...
{
    mControl->setFmtTriggerCriterion(index);
    return;
}
