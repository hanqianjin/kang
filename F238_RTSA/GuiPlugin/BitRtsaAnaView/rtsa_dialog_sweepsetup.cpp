...
RtSa_Dialog_SweepSetup::RtSa_Dialog_SweepSetup(RtSa_Interface *interface, RtSa_Dialog_Digitinput *input, QWidget *parent) :
    QWidget(parent),
...
//    QString button_style= "QPushButton:pressed{background-image: url(:/rtsa/image/fre_checked_right.png);\
//                            background-position:right;}"
//            "QPushButton:!pressed{background-image: url(:/rtsa/image/fre_checked_left.png); \
//                                background-position:right;}";
...
//        ui->pb_Spectrogram->setChecked(false);
    ...
    else if(mControl->getDisplay() == RTSA_SPECTROGRAM)
    {
...
//        return;
    ...
    mControl->setSingleSweep(RTSA_CONT);
    ui->pb_single->setChecked(mControl->IsSingleSweep());
    return;
}

void RtSa_Dialog_SweepSetup::on_pb_single_clicked()
{
    mControl->setSingleSweep(RTSA_SINGLE);
    ui->pb_conti->setChecked(!mControl->IsSingleSweep());
    ui->pb_single->setChecked(true);
    return;
}
...
//        return;
//    }
    mControl->setPersistenceTimeInfinite(RTSA_INFINITE);
    return;
}
...
//        return;
//    }
    mControl->setPersistenceTimeInfinite(RTSA_FINITE);
    return;
}
