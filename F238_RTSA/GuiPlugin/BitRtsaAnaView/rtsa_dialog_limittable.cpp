...
RtSa_Dialog_LimitTable::RtSa_Dialog_LimitTable(RtSa_Interface *interface, RtSa_Dialog_Digitinput *mInput, QWidget *parent) :
    QWidget(parent),
...
{
    qint32 limit = mControl->getCurLimit();
    QTableWidgetItem *item=ui->tableWidget->item(row,column);
    ui->tableWidget->editItem(item);
    mControl->SetCurLimitDataIndex(row);
...
void RtSa_Dialog_LimitTable::initTable()
{
    ...
    index = mControl->getCurLimit();
    size = mControl->getLimitDataNum(index);
    qreal* freq = mControl->getLimitFreqData(index);
    qreal* ampt = mControl->getLimitAmptData(index);
...
void RtSa_Dialog_LimitTable::resetTableSort(qint32 rowIndex)
{
    ui->tableWidget->judgeTableContent(ui->tableWidget,mControl->getStartFreq(),mControl->getStopFreq());
    ui->tableWidget->resetAllParam();
    mControl->SetCurLimitDataIndex(rowIndex);
...
void RtSa_Dialog_LimitTable::refreshEditDisplay(quint32 index)
{
    ...
    case RTSA_INPUT_LIMIT_FREQ:
        initTable();
        value = mControl->getLimitCurPointFreq(mControl->GetCurLimitDataIndex(),mControl->getCurLimit());
        strGab = QString::number((qreal) value,'f',2);
        ui->tableWidget->setItem(ui->tableWidget->currentRow(),0,new QTableWidgetItem(strGab));
        refreshLimitData();

        ui->tableWidget->setCurrentCell(mControl->GetCurLimitDataIndex(),0);
        break;
    case RTSA_INPUT_LIMIT_AMPT:
        value = mControl->getLimitCurPointAmpt(mControl->GetCurLimitDataIndex(),mControl->getCurLimit());
...
//        strGab = mControl->getRefLevelAdjustStr();
        ...
        ui->tableWidget->setCurrentCell(mControl->GetCurLimitDataIndex(),1);
        break;
...
{
    qint32 index = mControl->GetCurLimitDataIndex() + 1;
