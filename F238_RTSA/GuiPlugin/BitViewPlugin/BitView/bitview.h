...
//  void drawSpectrogram(QPainter* painter);  //main spectrogram func.
  ...
  //Albert for spectrogram data deal, in order to void TRACE_OFF state
  void oriSpectrogramDataDeals(qreal *pBuffer,quint32 dataNum);
...
//  float4 add4(float4 a, float4 b);
  ...
  qreal freqIndexToCenterFreq(qint32 index);
  qreal IndexToRefLevel(qint32 index);
  //Albert 19/11/18
  void renderDoubleClickPixMap();
  //Albert render tag line
//  void renderTagline();
  void renderSpectrogramCursorIndex(QPainter *painter);
  void resetDensityLineData();
  bool renderDensityLines(QPainter *painter);
  bool isSpectrogramCursorEnable();
...
signals:
  ...
  //Albert 19/10/29 for F239 touch scale resize
#ifdef BITVIEW_F239
  void scaleResize(qreal xsize,qreal ysize);
  void suspendDraw();
  void ChangeCenterFreq(qreal freq);
  void ChangeRefLevel(qreal refLevel);
#endif
private slots:
...
#ifdef BITVIEW_F239
  QTimer* Touchtimer;
  qreal BitXScale,BitYScale;
  QPixmap gDoubleClickPixMap;
  qint32 nFingerTouchCnt; //this cnt used for 2-finger move ntimes/respond.
  qreal DrawXScale;
  BIT_WINDOW_POS gWindowPos;
  bool misBitCenFreqChanged;
  bool misBitScaleChanged;
  bool misTouchEventDone;
  bool misBitReflevelChanged;
#endif
  qint32 clickCnt;
  bool mFileLoadFlag;
  qint32 mDensityArrayStartPos;
  qint32 mDensityArrayTotalNum;
  bool mIsDensityArrayReset;
};
